...
export type ApprovalRuntimeNode = { id: string; stepNo: number; nodeStatus: string; approvalMode?: string; stepPolicy?: string; assigneeType?: string; assigneeId?: string; assigneeName?: string; sourceType?: string; sourceValue?: string; conditionText?: string; slaHours?: number; dueAt?: string; remindedAt?: string; escalatedAt?: string; completedAt?: string; approverName?: string; approvalComment?: string };
export type ExpenseLine = { id?: string; lineNo?: number; expenseType: ExpenseType; amount: number; expenseDate: string; description: string; invoiceFileName?: string; invoiceContentType?: string; invoiceSizeBytes?: number };
export type Expense = { id: string; code: string; claimantId?: string; claimantName: string; projectId?: string; projectCode?: string; workOrderId?: string; workOrderCode?: string; expenseType: ExpenseType; amount: number; expenseDate: string; description: string; status: ExpenseStatus; approvalRequestId: string; lines?: ExpenseLine[] };
...
export type Approval = { id: string; code: string; approvalType: ApprovalType; title: string; sourceNo?: string; amount: number; status: ApprovalStatus; applicantName: string; content: string; approverName?: string; approvalComment?: string; processedAt?: string; createdAt: string; departmentName?: string; businessType?: string; projectCode?: string; supplierRisk?: string; customerLevel?: string; approvalMode?: string; currentStep?: number; totalSteps?: number; currentApproverName?: string; matchedRuleText?: string; approvalConfigVersion?: number; approvalPlanSnapshot?: string; sourceDetail?: Expense | Outsource | Record<string, unknown> | null; nodes?: ApprovalRuntimeNode[]; actions: Array<{ id: string; decision: ApprovalStatus; operatorName: string; comment: string; actionType?: string; stepNo?: number; createdAt: string }> };
...
export function returnApproval(id: string, data: { comment: string; operatorName: string }) { return request<Approval>({ method: "POST", url: `/office/approvals/${id}/return`, data }); }
export function resubmitApproval(id: string, data: { comment: string; applicantName: string }) { return request<Approval>({ method: "POST", url: `/office/approvals/${id}/resubmit`, data }); }
export function listExpenses() { return request<Expense[]>({ method: "GET", url: "/office/expenses" }); }
export function createExpense(data: { code: string; claimantId?: string; claimantName: string; projectId?: string; workOrderId?: string; expenseType?: ExpenseType; amount?: number; expenseDate?: string; description?: string; lines?: ExpenseLine[] }) { return request<Expense>({ method: "POST", url: "/office/expenses", data }); }
