    ...
    >
      ...
      >
        ...
        <a-sub-menu v-if="canAccessCrm" key="crm">
          ...
          <a-menu-item
            v-if="auth.can('crm:dashboard:view')"
            key="/crm/dashboard"
            >CRM仪表盘</a-menu-item
          >
          <a-menu-item v-if="auth.can('crm:customer:view')" key="/crm/customers"
            >客户池</a-menu-item
          >
          <a-menu-item
            v-if="auth.can('crm:opportunity:view')"
            key="/crm/opportunities"
            >线索商机</a-menu-item
          >
          <a-menu-item v-if="auth.can('crm:quote:view')" key="/crm/quotes"
            >报价方案</a-menu-item
          >
          <a-menu-item v-if="auth.can('crm:contract:view')" key="/crm/contracts"
            >客户合同</a-menu-item
          >
          <a-menu-item
            v-if="auth.can('crm:followup:view')"
            key="/crm/follow-ups"
            >跟进回访</a-menu-item
          >
          <a-menu-item v-if="auth.can('crm:renewal:view')" key="/crm/renewals"
            >续约管理</a-menu-item
          >
          <a-menu-item
            v-if="auth.can('crm:receivable:view')"
            key="/crm/receivables"
            >合同应收</a-menu-item
          >
        </a-sub-menu>
        ...
        <a-sub-menu v-if="canAccessHumanResources" key="hr">
          ...
          <a-menu-item
            v-if="auth.can('qualification:employee:view')"
            key="/hr/lifecycle"
            >入转调离</a-menu-item
          >
          <a-menu-item v-if="auth.can('workforce:view')" key="/hr/leaves"
            >请假管理</a-menu-item
          >
          <a-menu-item
            v-if="auth.can('qualification:employee:view')"
        ...
        <a-sub-menu v-if="canAccessQualification" key="qualification">
          ...
          <a-menu-item
            v-if="auth.can('qualification:view')"
            key="/qualification/dashboard"
            >资质总览</a-menu-item
          >
          <a-menu-item
            v-if="auth.can('qualification:company:view')"
            key="/qualification/companies"
            >公司资质</a-menu-item
          >
          <a-menu-item
            v-if="auth.can('qualification:tender:view')"
            key="/qualification/tender"
            >投标查询</a-menu-item
          >
          <a-menu-item
            v-if="auth.can('qualification:warning:view')"
            key="/qualification/warnings"
            >预警中心</a-menu-item
          >
        </a-sub-menu>
        <a-sub-menu v-if="canAccessOffice" key="office">
          ...
          <a-menu-item
            v-if="auth.can('office:expense:view')"
            key="/office/expenses"
            >费用报销</a-menu-item
          >
          <a-menu-item
          ...
          >
          <a-menu-item
            v-if="auth.can('office:document:view')"
            key="/office/documents"
            >电子档案</a-menu-item
          >
          <a-menu-item
            v-if="auth.can('office:notification:view')"
            key="/office/notifications"
            >消息中心</a-menu-item
          >
          <a-menu-item v-if="auth.can('office:audit:view')" key="/office/audits"
            >操作审计</a-menu-item
          >
        </a-sub-menu>
        <a-sub-menu v-if="canAccessFinance" key="finance">
          ...
          <a-menu-item v-if="auth.can('finance:view')" key="/finance/overview"
            >资金概览</a-menu-item
          >
          <a-menu-item
          ...
          >
          <a-menu-item
            v-if="canAccessPaymentApplications"
            key="/finance/payment-applications"
            >付款申请</a-menu-item
          >
          <a-menu-item
        ...
        <a-sub-menu
          v-if="
            ...
            auth.can('system:permission:view') ||
            auth.can('system:deleted-records:manage') ||
            auth.can('risk:update')
          "
          key="system"
        >
          ...
          >
          <a-menu-item
            v-if="auth.can('system:organization:view')"
            key="/system/organizations"
            >组织架构</a-menu-item
          >
          <a-menu-item v-if="auth.can('system:role:view')" key="/system/roles"
            >角色管理</a-menu-item
          >
          <a-menu-item
            v-if="auth.can('system:permission:view')"
            key="/system/permissions"
            >权限管理</a-menu-item
          >
          <a-menu-item
            v-if="auth.can('system:role:view')"
            key="/system/approval-configs"
            >审批人员配置</a-menu-item
          >
          <a-menu-item
            v-if="auth.can('system:role:view') || auth.can('risk:update')"
            key="/system/process-rules"
            >流程规则配置</a-menu-item
          >
          <a-menu-item
            v-if="auth.can('system:deleted-records:manage')"
            key="/system/deleted-records"
            >删除回收站</a-menu-item
          >
        </a-sub-menu>
        <a-sub-menu key="self">
...
const canAccessPaymentApplications = computed(() =>
  [
...
const canAccessQualification = computed(() =>
  [
    "qualification:view",
    "qualification:company:view",
    "qualification:tender:view",
    "qualification:warning:view",
  ].some((permission) => auth.can(permission)),
);
const canAccessOffice = computed(() =>
  [
    "office:view",
    "office:expense:view",
    "office:outsource:view",
