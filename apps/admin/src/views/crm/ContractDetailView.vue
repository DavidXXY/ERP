  ...
  <div class="page-stack">
    <a-card>
      <template #extra>
        <a-space>
          ...
          >
          <a-button
            v-if="canSubmitSignedApproval"
            type="primary"
            @click="handleSubmitSignedApproval"
            >提交盖章件审批</a-button
          >
          <a-button
            v-if="canApproveSignedDoc"
            type="primary"
            @click="handleApproveSignedDocument"
            >盖章件审批通过</a-button
          >
          <a-button
            v-if="canApproveSignedDoc"
            danger
            @click="handleRejectSignedDocument"
            >盖章件审批驳回</a-button
          >
        </a-space>
      </template>
      ...
      <template v-else-if="record">
        <div class="print-area">
          ...
          >
            ...
            <a-descriptions-item label="关联报价">{{
              record.quoteId || "未关联"
            }}</a-descriptions-item>
          </a-descriptions>
          ...
          >
            <template #extra>
              <a-space>
                <a-tag :color="closureRisk.color">{{
                  closureRisk.label
                }}</a-tag>
                <a-button size="small" @click="loadClosureData"
                  >刷新闭环</a-button
                >
              </a-space>
            </template>
            ...
            <a-row :gutter="[12, 12]" class="closure-stats">
              ...
              >
                <div class="closure-stat">
                  ...
                  <strong :class="{ danger: item.danger }">{{
                    item.value
                  }}</strong>
                  <p>{{ item.hint }}</p>
                </div>
            ...
            <a-list
              class="closure-actions"
              size="small"
              :data-source="closureActions"
            >
              <template #header>
                <strong>建议动作</strong>
              </template>
          ...
          <a-alert
            ...
            :message="
              record.status === 'PENDING_APPROVAL'
                ? '合同待审批'
                : record.status === 'PENDING_SEAL'
                  ? '合同已审批，待上传双方盖章件'
                  : record.status === 'SEAL_APPROVAL'
                    ? '双方盖章件审批中'
                    : '合同未生效'
            "
          ...
          >
            <template #action>
              <a-space>
                ...
                <a-button
                  v-if="record.status === 'SEAL_APPROVAL'"
                  size="small"
                  @click="contractTabKey = 'signed'"
                >
                  查看盖章件
                </a-button>
                <a-button
          ...
          <a-card title="开票申请" style="margin-top: 16px">
            <template #extra>
              <span class="table-subtitle"
                >按合同应收节点逐笔申请，财务审核后登记发票</span
              >
            </template>
            <a-empty
              v-if="contractReceivables.length === 0"
              description="当前合同暂无应收计划，暂不能申请开票"
            />
            <a-table
              v-else
              :data-source="contractReceivables"
              :pagination="false"
              :row-key="(item: Receivable) => item.id"
              size="small"
              :scroll="{ x: 1240 }"
            >
              <a-table-column title="应收单" data-index="code" width="170" />
              <a-table-column
                title="客户名称"
                data-index="customerName"
                width="180"
              />
              <a-table-column title="合同名称 / 编号" width="240">
                <template #default="{ record: item }">
                  <strong>{{
                    item.contractName || record?.projectName
                  }}</strong>
                  <span class="table-subtitle">{{
              ...
              <a-table-column title="开票状态" width="190">
                <template #default="{ record: item }">
                  <a-tag :color="invoiceStatusColor(item)">{{
                    invoiceStatusLabel(item)
                  }}</a-tag>
                  <span
                    v-if="
                      item.invoiceReviewComment &&
                      item.invoiceRequestStatus === 'REJECTED'
                    "
                    class="table-subtitle text-danger"
                    >{{ item.invoiceReviewComment }}</span
                  >
                </template>
              </a-table-column>
              <a-table-column title="操作" width="130" fixed="right">
                <template #default="{ record: item }">
                  ...
                  >
                  <span v-else class="table-subtitle">{{
                    invoiceNextAction(item)
                  }}</span>
                </template>
              </a-table-column>
            </a-table>
          ...
          <a-card v-if="relatedQuote" title="关联报价" style="margin-top: 16px">
            <a-descriptions bordered :column="{ xs: 1, sm: 2, md: 3 }">
              <a-descriptions-item label="报价编号">{{
                relatedQuote.code
              }}</a-descriptions-item>
                ...
                <strong>{{
                  formatMoney(
                    relatedQuote.netAmount ??
                      calcNetAmount(relatedQuote.amount, relatedQuote.taxRate),
                  )
                }}</strong>
              </a-descriptions-item>
              <a-descriptions-item label="服务范围" :span="3">{{
                relatedQuote.serviceScope
              }}</a-descriptions-item>
              <a-descriptions-item label="服务频次">{{
                relatedQuote.inspectCycle || "未设置"
              }}</a-descriptions-item>
              <a-descriptions-item label="付款方式/节点">{{
                relatedQuote.paymentNodes || "未设置"
              }}</a-descriptions-item>
              <a-descriptions-item label="状态">
                <a-tag :color="quoteStatusColor(relatedQuote.status)">{{
                  quoteStatusLabel(relatedQuote.status)
                }}</a-tag>
              </a-descriptions-item>
              <a-descriptions-item
                v-if="relatedQuote.lastApproverName"
                label="审批人"
                >{{ relatedQuote.lastApproverName }}</a-descriptions-item
              >
              <a-descriptions-item
                v-if="relatedQuote.lastApprovalComment"
                label="审批意见"
                :span="2"
                >{{ relatedQuote.lastApprovalComment }}</a-descriptions-item
              >
            </a-descriptions>
          </a-card>

          <a-card
            v-if="relatedOpportunity"
            title="关联商机"
            style="margin-top: 16px"
          >
            <a-descriptions bordered :column="{ xs: 1, sm: 2, md: 3 }">
              ...
              <a-descriptions-item label="成功率"
                >{{ relatedOpportunity.probability }}%</a-descriptions-item
              >
    ...
    <a-tabs v-model:activeKey="contractTabKey" style="margin-top: 16px">
      ...
      <a-tab-pane key="approval" tab="审批件">
        <template #extra>
          <a-upload
            :before-upload="(f: File) => handleUpload('APPROVAL_DOC', f)"
            :show-upload-list="false"
        ...
        <a-empty
          v-if="approvalAttachments.length === 0"
          description="暂无审批件"
        />
          ...
          <template #renderItem="{ item }">
            <a-list-item>
              <a-button
                type="link"
                size="small"
                @click="handleOpenAttachment(item)"
      ...
      <a-tab-pane key="signed" tab="双方盖章件">
        ...
        <a-empty
          v-if="signedAttachments.length === 0"
          description="暂无盖章件"
        />
      ...
      <a-tab-pane key="changes" tab="变更记录">
        ...
        <div v-else class="change-timeline">
          <div
            ...
            :key="item.id"
            class="change-card"
            :class="'change-' + (item.status?.toLowerCase() || 'pending')"
          >
            ...
            <div v-if="item.status === 'PENDING'" style="margin-top: 8px">
              <a-space>
                <a-button
                  size="small"
                  type="primary"
                  @click="handleApproveChange(item.id)"
                  >通过</a-button
                >
                <a-button
                  size="small"
                  danger
                  @click="handleRejectChange(item.id)"
                  >驳回</a-button
                >
              </a-space>
            </div>
    ...
    <a-modal
      v-model:open="invoiceRequestOpen"
      title="发起合同开票申请"
      width="520px"
    ...
    >
      <a-descriptions
        ...
        class="section-alert"
      >
        <a-descriptions-item label="销售合同"
          >{{ record?.projectName }} · {{ record?.code }}</a-descriptions-item
        >
        <a-descriptions-item label="客户名称">{{
          selectedReceivable.customerName
        }}</a-descriptions-item>
        <a-descriptions-item label="应收节点"
      ...
      <a-alert
        type="info"
        show-icon
        message="提交后由财务审核；审核通过后财务登记正式发票。"
        class="section-alert"
      />
      <a-form layout="vertical">
        <a-form-item label="申请说明（可选）">
          <a-textarea
            v-model:value="invoiceRequestForm.remark"
            :rows="3"
            placeholder="例如：客户要求本月开票，请注明开票节点或特殊要求"
          />
        </a-form-item>
      </a-form>
    </a-modal>
        ...
        <a-row :gutter="16">
          <a-col :xs="24" :md="12"
            ><a-form-item label="项目名称" name="projectName"
              ><a-input v-model:value="editForm.projectName" /></a-form-item
          ></a-col>
          <a-col :xs="24" :md="12"
          ...
          <a-col :xs="24" :md="12"
            ><a-form-item label="服务频次"
              ><a-input v-model:value="editForm.serviceCycle" /></a-form-item
          ></a-col>
          <a-col :xs="24" :md="12"
          ...
          <a-col :span="24"
            ><a-form-item label="变更原因" name="reason"
              ><a-textarea
                ...
                :rows="2"
                placeholder="请说明变更原因" /></a-form-item
          ></a-col>
        </a-row>
      </a-form>
    </a-modal>
      ...
      try {
        ...
        if (relatedQuote.value?.opportunityId) {
          try {
            ...
            /* opportunity fetch is supplementary */
          }
        }
      } catch {
        /* quote fetch is supplementary */
      }
    }
...
function openInvoiceRequest(item: Receivable) {
  selectedReceivable.value = item;
  invoiceRequestForm.remark = "";
  invoiceRequestOpen.value = true;
}
...
async function handleInvoiceRequest() {
  ...
  saving.value = true;
  try {
    await applyReceivableInvoice(selectedReceivable.value.id, {
      applicantName: auth.user?.displayName || "当前用户",
      remark: invoiceRequestForm.remark || undefined,
    });
    invoiceRequestOpen.value = false;
    message.success("开票申请已提交财务审核");
    await loadClosureData();
  } catch (error) {
    message.error(error instanceof Error ? error.message : "开票申请提交失败");
  } finally {
    saving.value = false;
  }
}
...
function invoiceNextAction(item: Receivable) {
  if (item.invoiceNo) return item.invoiceNo;
  if (item.invoiceRequestStatus === "PENDING_APPROVAL") return "等待财务审核";
  if (item.invoiceRequestStatus === "APPROVED") return "等待财务开票";
  return "不可申请";
}

const receivableTotal = computed(() =>
  contractReceivables.value.reduce(
...
const invoicedTotal = computed(() =>
  contractReceivables.value.reduce(
    (sum, item) => sum + (item.invoiceNo ? Number(item.amount || 0) : 0),
    0,
  ),
);
...
const overdueReceivables = computed(() =>
  contractReceivables.value.filter((item) => item.status === "OVERDUE"),
);
const collectionRate = computed(() =>
  receivableTotal.value > 0
    ? Math.round((receivedTotal.value / receivableTotal.value) * 1000) / 10
    : 0,
);
...
const closureRisk = computed(() => {
  if (
    overdueReceivables.value.length > 0 ||
    outstandingTotal.value > Number(record.value?.amount || 0) * 0.3
  )
    return { label: "高风险", color: "red" };
  if (
    !relatedProject.value ||
    contractReceivables.value.length === 0 ||
    daysToEnd.value <= 30
  )
    return { label: "需关注", color: "orange" };
...
const closureStats = computed(() => [
  {
    key: "progress",
    label: "合同进度",
    value: `${contractProgress.value}%`,
    hint:
      daysToEnd.value >= 0
        ? `距到期 ${daysToEnd.value} 天`
        : `已到期 ${Math.abs(daysToEnd.value)} 天`,
    danger: daysToEnd.value <= 30,
  },
  {
    key: "project",
    label: "项目承接",
    value: relatedProject.value ? "已承接" : "未承接",
    hint: relatedProject.value?.code || "合同生效后建议生成项目",
    danger: !relatedProject.value && record.value?.status === "ACTIVE",
  },
  {
    key: "collection",
    label: "回款率",
    value: `${collectionRate.value}%`,
    hint: `${formatMoney(receivedTotal.value)} / ${formatMoney(receivableTotal.value)}`,
    danger: overdueReceivables.value.length > 0,
  },
  {
    key: "outstanding",
    label: "未回款",
    ...
    hint: overdueReceivables.value.length
      ...
      : "应收状态正常",
    danger: outstandingTotal.value > 0,
  },
]);
const closureSteps = computed(() => [
  {
    key: "customer",
    label: "客户",
    title: record.value?.customerName || "-",
    hint: "客户档案与开票信息",
    done: Boolean(record.value?.customerId),
    warn: false,
    route: record.value?.customerId
      ? `/crm/customers?customer=${record.value.customerId}`
      : "",
  },
  {
    key: "opportunity",
    ...
    route: relatedOpportunity.value
      ? `/crm/opportunities/${relatedOpportunity.value.id}`
      : "/crm/opportunities",
  },
  {
    key: "quote",
    label: "报价",
    title: relatedQuote.value?.code || "未关联",
    hint: relatedQuote.value
      ? `V${relatedQuote.value.versionNo}`
      : "报价利润校验不可追溯",
    done: Boolean(relatedQuote.value),
    warn: !relatedQuote.value,
    route: relatedQuote.value
      ? `/crm/quotes/${relatedQuote.value.id}`
      : "/crm/quotes",
  },
  {
    key: "contract",
    label: "合同",
    title: record.value?.code || "-",
    hint: contractStatusLabel(record.value?.status || "ACTIVE"),
    done: Boolean(record.value),
    warn: false,
    route: "",
  },
  {
  ...
  },
  {
    key: "receivable",
    label: "应收回款",
    title: `${contractReceivables.value.length} 笔`,
    hint: `${collectionRate.value}% 已回款`,
    done: contractReceivables.value.length > 0,
    warn:
      contractReceivables.value.length === 0 ||
      overdueReceivables.value.length > 0,
...
const closureActions = computed(() => {
  ...
  if (record.value?.status === "ACTIVE" && !relatedProject.value) {
    rows.push({
      ...
      title: "合同已生效但未承接项目",
      description:
        "建议立即生成项目草稿，带入合同额、周期和默认预算，避免采购和成本无法归集。",
    });
  }
  if (contractReceivables.value.length === 0) {
    rows.push({
      ...
      level: "high",
      title: "缺少应收计划",
      description: "合同没有应收节点，财务无法跟踪开票、到期、回款和逾期。",
  ...
  if (!relatedQuote.value) {
    rows.push({
      ...
      level: "medium",
      title: "报价链路缺失",
      description: "缺少报价会影响毛利、折扣权限、成本项校验的追溯。",
      route: "/crm/quotes",
    });
  }
  if (daysToEnd.value <= 30) {
    rows.push({
      ...
      description:
        daysToEnd.value >= 0
          ? `距到期 ${daysToEnd.value} 天，建议提前进入续约评估。`
          : `已到期 ${Math.abs(daysToEnd.value)} 天，建议关闭或续约。`,
      route: "/crm/renewals",
    });
  }
  return rows;
...
const approvalAttachments = computed(() =>
  attachments.value.filter(
    (a) => a.attachmentType === "APPROVAL_DOC" || !a.attachmentType,
  ),
);
const signedAttachments = computed(() =>
  attachments.value.filter((a) => a.attachmentType === "SIGNED_DOC"),
);
const pendingSignedDocApproval = computed(
  () =>
    changeList.value.find(
      (item) =>
        item.status === "PENDING" &&
        (item.changeData || "").includes("SIGNED_DOC_APPROVAL"),
    ) || null,
);
const canApproveContract = computed(
  () =>
    record.value?.status === "PENDING_APPROVAL" &&
    approvalAttachments.value.length > 0 &&
    auth.can("crm:contract:update"),
);
const canUploadSignedDoc = computed(
  () =>
    record.value?.status === "PENDING_SEAL" && auth.can("crm:contract:update"),
);
const canSubmitSignedApproval = computed(
  () =>
    record.value?.status === "PENDING_SEAL" &&
    signedAttachments.value.length > 0 &&
    auth.can("crm:contract:update"),
);
const canApproveSignedDoc = computed(
  () =>
    record.value?.status === "SEAL_APPROVAL" &&
    Boolean(pendingSignedDocApproval.value) &&
    auth.can("crm:contract:update"),
);
  ...
  return [
    ...
    },
    {
      ...
      personName:
        ...
        (item.status === "ACTIVE" ? "盖章审批人" : "当前审批人"),
      title:
        item.status === "SEAL_APPROVAL"
          ? "待审批"
          : sealRejected
            ? "已驳回"
            : item.status === "ACTIVE"
              ...
              : "待提交",
      time:
        pendingSignedDocApproval.value?.approvedAt ||
        pendingSignedDocApproval.value?.createdAt,
      note:
        item.status === "SEAL_APPROVAL"
          ? "等待盖章件审批"
          : pendingSignedDocApproval.value?.approvalComment ||
            (item.status === "ACTIVE"
              ? "双方盖章件已审批"
              : "上传盖章件后审批"),
      state:
    ...
    },
    {
      ...
      state: item.status === "ACTIVE" ? "done" : "waiting",
    },
  ];
}

function contractChangeApprovalSteps(
  item: ContractChangeResponse,
  ...
  return [
    ...
    },
    {
      ...
      note:
        item.status === "PENDING"
          ? "等待合同变更审批"
          : item.approvalComment || item.reason,
      state:
        item.status === "PENDING"
          ? "pending"
          : item.status === "REJECTED"
...
async function handleSubmitSignedApproval() {
  try {
    await submitSignedDocumentApproval(id, {
      operatorName: auth.user?.displayName || "",
      comment: "双方盖章件审批",
    });
    ...
    message.error(
      error instanceof Error ? error.message : "提交盖章件审批失败",
    );
  }
}
...
async function handleApproveSignedDocument() {
  ...
  try {
    await approveContractChange(pendingSignedDocApproval.value.id, {
      operatorName: auth.user?.displayName || "",
      comment: "双方盖章件审批通过",
    });
...
async function handleRejectSignedDocument() {
  if (!pendingSignedDocApproval.value) return;
  try {
    await rejectContractChange(pendingSignedDocApproval.value.id, {
      operatorName: auth.user?.displayName || "",
      comment: "双方盖章件审批驳回",
    });
  ...
  try {
    const payload = {
      changeData: JSON.stringify({
        ...
        serviceCycle: editForm.serviceCycle,
...
async function loadChanges() {
  ...
  try {
    changeList.value = await listContractChanges(id);
  } catch {
  } finally {
...
async function handleApproveChange(changeId: string) {
  try {
    await approveContractChange(changeId, {
      operatorName: auth.user?.displayName || "",
      comment: "",
    });
...
async function handleRejectChange(changeId: string) {
  try {
    await rejectContractChange(changeId, {
      operatorName: auth.user?.displayName || "",
      comment: "",
    });
