<template>
  <div class="page-stack">
      ...
      <header class="customer-directory-header">
        <div>
          ...
          <div class="customer-summary">
            <span
              ><strong>{{ strategicCount }}</strong> 家战略客户</span
            >
            <span
        ...
        <a-space>
          ...
          >
          <a-button
            v-if="auth.can('crm:customer:create')"
            type="primary"
            @click="openCreate"
          >
        ...
        >
          <template #bodyCell="{ column, record }">
            ...
            <template v-else-if="column.key === 'owner'"
              ><strong>{{ record.ownerName || "未分配" }}</strong></template
            >
            ...
            <template v-else-if="column.key === 'action'">
              <a-space :size="2">
                ...
                ></a-button>
                <a-button
                  v-if="auth.can('crm:customer:update')"
                  type="text"
                  size="small"
                  title="编辑客户"
                  aria-label="编辑客户"
                  @click.stop="openEdit(record.id)"
                ...
                >
                  <span @click.stop>
                    <a-button type="text" size="small" danger title="删除客户"
        ...
        <a-spin :spinning="loading">
          ...
          <div v-else class="customer-mobile-items">
            <article
              ...
              role="button"
              tabindex="0"
              @click="selectCustomer(record.id)"
              @keydown.enter.prevent="selectCustomer(record.id)"
            >
              <div class="customer-mobile-heading">
                ...
                <div>
                  ...
                  ><span>{{ record.code }} · {{ record.ownerName }}</span>
                </div>
                <RightOutlined aria-hidden="true" />
              </div>
              <div class="customer-mobile-tags">
                <a-tag :color="levelColor(record.level)">{{
                  levelLabel(record.level)
                }}</a-tag
                ><a-tag :color="riskColor(record.riskStatus)">{{
              ...
              <div class="customer-mobile-meta">
                ...
                ><span
                  >项目地址 <strong>{{ record.siteCount }}</strong></span
                ><span class="customer-mobile-payment">{{
                  record.paymentHabit || "未登记付款习惯"
                }}</span>
              </div>
            </article>
    ...
    <a-drawer
      ...
      @after-open-change="handleDrawerVisibility"
    >
      <template #title>
        <div v-if="selectedDetail" class="customer-drawer-title">
          <span class="customer-avatar" aria-hidden="true">{{
            customerInitial(selectedDetail.name)
          }}</span>
          <div>
            <strong>{{ selectedDetail.name }}</strong
            ><span
              >{{ selectedDetail.code }} · {{ selectedDetail.industry }}</span
            >
          </div>
      ...
      <template #extra>
        <a-space v-if="selectedDetail">
          <template v-if="editing">
            ...
            <a-button type="primary" :loading="saving" @click="saveCustomer"
              >保存</a-button
            >
          </template>
          <template v-else>
            <a-button
              ...
              ><template #icon><PlusOutlined /></template>新增跟进</a-button
            >
            <a-button
              v-if="auth.can('crm:customer:update')"
              @click="openTransferOwner"
              ><template #icon><SwapOutlined /></template>转交负责人</a-button
            >
            <a-button
              v-if="auth.can('crm:customer:update')"
              type="primary"
              @click="openEdit(selectedDetail.id)"
              ><template #icon><EditOutlined /></template>编辑档案</a-button
            >
          </template>
        </a-space>
      </template>
      ...
      <a-spin :spinning="detailLoading">
        <template v-if="selectedDetail">
          <div v-if="!editing" class="customer-metric-band">
            ...
            <div>
              ...
              ><strong
                :class="{
                  'customer-risk-value':
                    selectedDetail.metrics.outstandingAmount > 0,
                }"
                >{{
                  formatMoney(selectedDetail.metrics.outstandingAmount)
                }}</strong
              >
            </div>
            <div>
          ...
          >
            <a-tab-pane key="profile" tab="客户档案">
              <div class="customer-detail-grid">
                <section class="customer-detail-section">
                  ...
                  <a-descriptions :column="2" size="small" bordered>
                    ...
                    <a-descriptions-item label="客户等级">{{
                      levelLabel(selectedDetail.level)
                    }}</a-descriptions-item>
                    ...
                    <a-descriptions-item label="付款习惯" :span="2">{{
                      selectedDetail.paymentHabit || "-"
                    }}</a-descriptions-item>
                ...
                <section class="customer-detail-section">
                  ...
                  <a-descriptions :column="2" size="small" bordered>
                    ...
                    <a-descriptions-item label="纳税人识别号" :span="2">{{
                      selectedDetail.invoice.taxNo || "-"
                    }}</a-descriptions-item>
                    <a-descriptions-item label="开户银行" :span="2">{{
                      selectedDetail.invoice.bankName || "-"
                    }}</a-descriptions-item>
                    <a-descriptions-item label="银行账号" :span="2">{{
                      selectedDetail.invoice.bankAccount || "-"
                    }}</a-descriptions-item>
                    <a-descriptions-item label="注册地址" :span="2">{{
                      selectedDetail.invoice.registeredAddress || "-"
                    }}</a-descriptions-item>
                    <a-descriptions-item label="注册电话" :span="2">{{
                      selectedDetail.invoice.registeredPhone || "-"
                    }}</a-descriptions-item>
                  </a-descriptions>
                </section>
                <section class="customer-detail-section">
                  ...
                  <a-empty
                    v-if="!selectedDetail.contacts.length"
                    description="暂无联系人"
                  />
                  <div v-else class="customer-record-list">
                    <div
                      v-for="contact in selectedDetail.contacts"
                      :key="contact.id"
                      class="customer-record-row"
                    >
                      <div>
                        <strong>{{ contact.name }}</strong
                        ><a-tag v-if="contact.primaryContact" color="blue"
                          >主要联系人</a-tag
                        ><span>{{ contact.title || "未登记职务" }}</span>
                      </div>
                      <div>
                        <span>{{ contact.phone || "未登记电话" }}</span
                        ><span>{{ contact.email || "未登记邮箱" }}</span>
                      </div>
                    </div>
                  </div>
                </section>
                <section class="customer-detail-section">
                  ...
                  <a-empty
                    v-if="!selectedDetail.sites.length"
                    description="暂无项目地址"
                  />
                  <div v-else class="customer-record-list">
                    <div
                      v-for="site in selectedDetail.sites"
                      :key="site.id"
                      class="customer-record-row customer-site-row"
                    >
            ...
            <a-tab-pane
              key="opportunities"
              :tab="`跟踪项目 ${selectedDetail.opportunities.length}`"
            >
              <a-table
                size="small"
                ...
                :pagination="false"
                :row-key="
                  (record: CustomerDetail['opportunities'][number]) => record.id
                "
                :scroll="{ x: 900 }"
              >
                <template #bodyCell="{ column, record }"
            ...
            <a-tab-pane
              key="contracts"
              :tab="`已签合同 ${selectedDetail.contracts.length}`"
            >
              <a-table
                size="small"
                :columns="contractColumns"
                :data-source="selectedDetail.contracts"
                :pagination="false"
                :row-key="
                  (record: CustomerDetail['contracts'][number]) => record.id
                "
                :scroll="{ x: 820 }"
              >
                <template #bodyCell="{ column, record }"
                  ><template v-if="column.key === 'contract'"
                    ...
                    ><span class="table-subtitle"
                      >{{ record.code }} · {{ record.contractType }}</span
                    ></template
                  ><template v-else-if="column.key === 'period'"
                    >{{ record.startDate }} 至 {{ record.endDate }}</template
                  ><template v-else-if="column.key === 'amount'">{{
            ...
            <a-tab-pane
              key="receivables"
              :tab="`合同应收 ${selectedDetail.receivables.length}`"
            >
              <a-table
                size="small"
                :columns="receivableColumns"
                :data-source="selectedDetail.receivables"
                :pagination="false"
                :row-key="
                  (record: CustomerDetail['receivables'][number]) => record.id
                "
                :scroll="{ x: 900 }"
              >
                <template #bodyCell="{ column, record }"
            ...
            <a-tab-pane
              key="followups"
              :tab="`跟进记录 ${selectedDetail.followUps.length}`"
            >
              <a-empty
                v-if="!selectedDetail.followUps.length"
                description="暂无跟进记录"
              />
              <div v-else class="customer-followup-list">
                <article
                  v-for="item in selectedDetail.followUps"
                  :key="item.id"
                  class="customer-followup-item"
                >
                  <header>
                    <div>
                      <a-tag :color="followUpTypeColor(item.type)">{{
                        followUpTypeLabel(item.type)
                      }}</a-tag
                      ><strong>{{ item.subject }}</strong>
                    </div>
                    <time>{{ formatDateTime(item.followedAt) }}</time>
                  </header>
                  <p>{{ item.content }}</p>
                  <footer>
                    <span
                      >{{ item.ownerName
                      }}<template v-if="item.opportunityCode">
                        · {{ item.opportunityCode }}</template
                      ></span
                    ><span>下一步：{{ item.nextAction || "未安排" }}</span>
                  </footer>
                </article>
              </div>
            </a-tab-pane>
            ...
            <a-row :gutter="16">
              ...
              <a-col :xs="24" :md="12"
                ><a-form-item label="行业" name="industry"
                  ><a-input v-model:value="formState.industry" /></a-form-item
              ></a-col>
              <a-col :xs="24" :md="8"
                ><a-form-item label="客户等级" name="level"
                  ><a-select
                    v-model:value="formState.level"
                    :options="levelOptions" /></a-form-item
              ></a-col>
              <a-col :xs="24" :md="8"
              ...
              <a-col :xs="24" :md="8"
                ><a-form-item label="风险状态"
                  ><a-select
                    v-model:value="formState.riskStatus"
                    :options="riskOptions" /></a-form-item
              ></a-col>
              <a-col :xs="24" :md="12"
                ><a-form-item label="付款习惯"
                  ><a-input
                    v-model:value="formState.paymentHabit" /></a-form-item
              ></a-col>
              <a-col :xs="24" :md="12"
                ><a-form-item label="风险说明"
                  ><a-textarea
                    v-model:value="formState.riskNote"
                    :rows="2" /></a-form-item
              ></a-col>
              ...
              <a-col :span="24"
                ><a-form-item label="发票抬头"
                  ><a-input
                    v-model:value="formState.invoiceTitle" /></a-form-item
              ></a-col>
              <a-col :xs="24" :md="12"
                ><a-form-item label="纳税人识别号"
                  ><a-input v-model:value="formState.taxNo" /></a-form-item
              ></a-col>
              <a-col :xs="24" :md="12"
                ><a-form-item label="注册电话"
                  ><a-input
                    v-model:value="formState.registeredPhone" /></a-form-item
              ></a-col>
              <a-col :xs="24" :md="12"
                ><a-form-item label="开户银行"
                  ><a-input v-model:value="formState.bankName" /></a-form-item
              ></a-col>
              <a-col :xs="24" :md="12"
                ><a-form-item label="银行账号"
                  ><a-input
                    v-model:value="formState.bankAccount" /></a-form-item
              ></a-col>
              <a-col :span="24"
                ><a-form-item label="注册地址"
                  ><a-input
                    v-model:value="formState.registeredAddress" /></a-form-item
              ></a-col>
            </a-row>
            ...
            <div style="margin-top: 16px">
              ...
              <div class="customer-form-list">
                <div
                  ...
                  :key="contact.key"
                  class="customer-contact-form-row"
                >
                  <label
                    >姓名<a-input
                  ...
                  <label class="customer-primary-switch"
                    >主要联系人<a-switch
                      :checked="contact.primaryContact"
                      @change="setPrimaryContact(index, Boolean($event))"
                  /></label>
                  <a-button
                    danger
                    type="text"
            ...
            <div style="margin-top: 16px">
              ...
              <div class="customer-form-list-header">
                ...
                <a-button @click="addSite"
                  ><template #icon><PlusOutlined /></template>添加地址</a-button
                >
              </div>
              <div class="customer-form-list">
                <div
                  v-for="(site, index) in formState.sites"
                  :key="site.key"
                  class="customer-site-form-row"
                >
                  <label
                    >地址名称<a-input
                      v-model:value="site.name"
                      placeholder="例如总部、项目现场"
                  /></label>
                  <label
                    >详细地址<a-input v-model:value="site.address"
                  /></label>
                  <a-button
                    danger
                    type="text"
    ...
    <a-modal
      ...
      :title="editingCustomerId ? '编辑客户档案' : '新增客户'"
      width="960px"
        ...
        <a-tabs v-model:active-key="formTab" class="customer-form-tabs">
          <a-tab-pane key="base" tab="基本资料">
            <a-row :gutter="16">
              ...
              <a-col :xs="24" :md="12"
                ><a-form-item label="付款习惯"
                  ><a-input
                    v-model:value="formState.paymentHabit"
                    placeholder="例如：月结30天、验收后付款" /></a-form-item
              ></a-col>
              <a-col :xs="24" :md="12"
                ><a-form-item label="风险状态"
                  ><a-select
          ...
          <a-tab-pane key="contacts" :tab="`联系人 ${validContactCount}`">
            <div class="customer-form-list-header">
              <h3>联系人</h3>
              <a-button @click="addContact"
                ><template #icon><PlusOutlined /></template>添加联系人</a-button
              >
            </div>
            <div class="customer-form-list">
              <div
    ...
    <a-modal
      ...
      width="460px"
      :confirm-loading="saving"
      @ok="handleTransferOwner"
    >
      <a-alert
        v-if="selectedDetail"
        class="customer-transfer-alert"
        type="info"
        show-icon
        :message="`${selectedDetail.name} · 当前负责人 ${selectedDetail.ownerName}`"
      />
      <a-form layout="vertical">
        <a-form-item label="新负责人" required>
          <a-select
            v-model:value="transferOwnerName"
            :options="userOptions"
            show-search
            option-filter-prop="label"
            placeholder="选择组织人员"
...
const userOptions = computed(() => {
  ...
  const selectedNames = [formState.ownerName, transferOwnerName.value].filter(
    Boolean,
  );
...
const validSiteCount = computed(
  () =>
    formState.sites.filter((site) => site.name.trim() && site.address.trim())
      .length,
);
...
async function loadCustomers() {
  ...
  try {
    ...
    if (
      queryCustomerId &&
      customers.value.some((customer) => customer.id === queryCustomerId)
    )
      await selectCustomer(queryCustomerId, false);
  } catch (error) {
    errorMessage.value =
...
async function selectCustomer(id: string, syncRoute = true) {
  ...
  try {
    ...
    if (syncRoute && route.query.customer !== id)
      await router.replace({
        path: route.path,
        query: { ...route.query, customer: id },
      });
  } catch (error) {
...
function customerRow(record: CustomerSummary) {
  return { onClick: () => selectCustomer(record.id) };
}
...
async function saveCustomer() {
  ...
  if (incompleteContact) {
    formTab.value = "contacts";
    message.error("请补充联系人姓名或删除该行");
    return;
  }
  const incompleteSite = formState.sites.find(
    (site) => Boolean(site.name.trim()) !== Boolean(site.address.trim()),
  );
  if (incompleteSite) {
    formTab.value = "sites";
    message.error("项目地址名称和详细地址需要同时填写");
    return;
  }
  ...
  try {
    ...
    message.error(error instanceof Error ? error.message : "客户档案保存失败");
  } finally {
    saving.value = false;
  }
}
...
function customerPayload(): UpdateCustomerPayload {
  return {
    ...
    invoice: {
      ...
      registeredPhone: formState.registeredPhone,
    },
    contacts: formState.contacts
      .filter((contact) => contact.name.trim())
      .map(({ key: _key, ...contact }) => contact),
    sites: formState.sites
      .filter((site) => site.name.trim() && site.address.trim())
      .map(({ key: _key, ...site }) => site),
  };
}

function resetForm(value: CustomerFormState) {
  Object.assign(formState, value);
}
function addContact() {
  formState.contacts.push(emptyContact());
}
function removeContact(index: number) {
  formState.contacts.splice(index, 1);
  if (!formState.contacts.length) formState.contacts.push(emptyContact());
  if (!formState.contacts.some((contact) => contact.primaryContact))
    formState.contacts[0].primaryContact = true;
}
function setPrimaryContact(index: number, checked: boolean) {
  formState.contacts.forEach((contact, contactIndex) => {
    contact.primaryContact = checked && contactIndex === index;
  });
}
function addSite() {
  formState.sites.push(emptySite());
}
function removeSite(index: number) {
  formState.sites.splice(index, 1);
  if (!formState.sites.length) formState.sites.push(emptySite());
}

function createFollowUpForCustomer() {
  if (selectedDetail.value)
    router.push({
      path: "/crm/follow-ups",
      query: { customer: selectedDetail.value.id, create: "1" },
    });
}
function handleDrawerVisibility(open: boolean) {
  if (!open && route.query.customer) {
...
function initialForm(ownerName = ""): CustomerFormState {
  return {
    code: "",
    name: "",
    industry: "",
...
function formFromDetail(detail: CustomerDetail): CustomerFormState {
  return {
    ...
    paymentHabit: detail.paymentHabit || "",
    riskStatus: detail.riskStatus,
    riskNote: detail.riskNote || "",
    invoiceTitle: detail.invoice.title || "",
    taxNo: detail.invoice.taxNo || "",
    bankName: detail.invoice.bankName || "",
    bankAccount: detail.invoice.bankAccount || "",
    registeredAddress: detail.invoice.registeredAddress || "",
    registeredPhone: detail.invoice.registeredPhone || "",
...
function emptyContact(primaryContact = false): ContactForm {
  return {
    key: uniqueKey(),
    name: "",
    title: "",
...
function emptySite(): SiteForm {
  return { key: uniqueKey(), name: "", address: "" };
}
function uniqueKey() {
...
<style scoped>
.customer-directory {
  container-name: customer-directory;
...
.customer-filter-bar :deep(.ant-input-prefix) {
  color: #84909c;
}
...
.customer-desktop-table :deep(.ant-table-thead > tr > th) {
  height: 44px;
  background: #fff;
  color: #596675;
  font-size: 12px;
...
.customer-desktop-table :deep(.ant-table-tbody > tr:hover > td) {
  background: #f5faf8;
}
.customer-name-cell {
  display: flex;
...
.customer-drawer-title > div {
  display: grid;
  min-width: 0;
}
.customer-drawer-title strong {
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
}
.customer-drawer-title span {
  color: #71808d;
  font-size: 12px;
  font-weight: 400;
}
.customer-drawer-title :deep(.ant-tag) {
  margin-inline-end: 0;
}
.customer-metric-band {
  display: grid;
...
.customer-metric-band div {
  min-width: 0;
  padding: 14px 16px;
  border-right: 1px solid #e3e8eb;
}
.customer-metric-band div:last-child {
  border-right: 0;
}
.customer-metric-band span,
.customer-metric-band strong {
  display: block;
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
...
.customer-detail-tabs :deep(.ant-tabs-nav) {
  margin-bottom: 18px;
}
...
.customer-record-row:last-child {
  border-bottom: 0;
}
.customer-record-row > div {
  display: flex;
  flex-wrap: wrap;
  gap: 7px;
  align-items: center;
...
.customer-record-row > div:last-child {
  justify-content: flex-end;
  color: #687582;
  font-size: 12px;
}
.customer-record-row :deep(.ant-tag) {
  margin-inline-end: 0;
}
.customer-record-row span {
  color: #687582;
  font-size: 12px;
}
.customer-site-row {
...
.customer-followup-item header,
.customer-followup-item footer {
  display: flex;
  align-items: center;
  justify-content: space-between;
  gap: 12px;
}
.customer-followup-item header > div {
  display: flex;
  align-items: center;
  gap: 6px;
}
.customer-followup-item :deep(.ant-tag) {
  margin-inline-end: 0;
}
.customer-followup-item time,
.customer-followup-item footer {
  color: #74818d;
  font-size: 12px;
}
...
.customer-contact-form-row {
  grid-template-columns: 1.1fr 1fr 1fr 1.3fr 88px 32px;
}
.customer-site-form-row {
  grid-template-columns: 180px minmax(0, 1fr) 32px;
}
.customer-contact-form-row label,
.customer-site-form-row label {
  display: grid;
  gap: 6px;
  color: #667481;
  font-size: 12px;
}
.customer-primary-switch {
  align-content: end;
  justify-items: start;
}
@container customer-directory (max-width: 820px) {
  ...
  .customer-mobile-item:hover,
  .customer-mobile-item:focus-visible {
    border-color: #8cbcaf;
    box-shadow: 0 2px 8px rgba(25, 72, 61, 0.08);
  }
  .customer-mobile-heading {
    display: grid;
    grid-template-columns: 36px minmax(0, 1fr) auto;
    gap: 10px;
    align-items: center;
  }
  .customer-mobile-heading > div {
    display: grid;
    min-width: 0;
  }
  .customer-mobile-heading strong,
  .customer-mobile-heading span {
    overflow: hidden;
    text-overflow: ellipsis;
    white-space: nowrap;
  }
  ...
  .customer-mobile-heading > svg {
    color: #9aa5af;
    font-size: 12px;
  }
  .customer-mobile-tags {
    display: flex;
    flex-wrap: wrap;
    align-items: center;
    gap: 5px;
  ...
  .customer-mobile-tags :deep(.ant-tag) {
    margin-inline-end: 0;
  }
  .customer-mobile-tags > span {
    color: #697683;
    font-size: 12px;
  }
...
@container customer-directory (max-width: 620px) {
  ...
  .customer-filter-bar > :first-child {
    grid-column: 1 / -1;
  }
  .customer-filter-result {
    grid-column: 1 / -1;
...
@media (max-width: 760px) {
  ...
  .customer-metric-band div:nth-child(2) {
    border-right: 0;
  }
  .customer-metric-band div:nth-child(-n + 2) {
  ...
  .customer-contact-form-row > button {
    justify-self: end;
  }
  .customer-site-form-row {
    grid-template-columns: 1fr 32px;
  }
  .customer-site-form-row label:nth-child(2) {
    grid-column: 1 / -1;
    grid-row: 2;
  }
  .customer-site-form-row > button {
    grid-column: 2;
    grid-row: 1;
  }
}
@media (max-width: 480px) {
  ...
  .customer-directory-header > .ant-space {
    width: 100%;
  }
  .customer-directory-header > .ant-space :deep(.ant-space-item:last-child) {
    flex: 1;
  }
  .customer-directory-header
    > .ant-space
