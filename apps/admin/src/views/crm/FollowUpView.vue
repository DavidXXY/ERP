    ...
    <a-card>
      ...
      <template v-if="activeTab === 'pending'">
        ...
        >
          <div class="fp-group-header">
            ...
            <span v-if="group.opportunityCode" class="table-subtitle">{{
              group.opportunityCode
            }}</span>
          </div>
          <div
            ...
            :key="item.id"
            class="fp-item"
            :class="{ 'fp-overdue': isOverdue(item.nextAction) }"
          >
            <div class="fp-item-head">
              <a-tag :color="followUpTypeColor(item.type)" size="small">{{
                followUpTypeLabel(item.type)
              }}</a-tag>
              <strong>{{ item.subject }}</strong>
              <a-tag v-if="isOverdue(item.nextAction)" color="red" size="small"
                >已逾期</a-tag
              >
            </div>
            ...
            <div class="fp-item-meta">
              <span
                >跟进人 {{ item.ownerName }} ·
                {{ formatDateTime(item.followedAt) }}</span
              >
              <span v-if="item.nextAction" class="fp-next-action">
                下一步：{{ item.nextAction }}
                <span v-if="item.nextAction" class="table-subtitle"
                  >（{{ item.nextAction }}）</span
                >
              </span>
            </div>
          </div>
      ...
      <template v-else>
        <a-table
          ...
          :loading="loading"
          :row-key="(record: FollowUp) => record.id"
          :scroll="{ x: 1180 }"
        >
          <template #bodyCell="{ column, record }">
            <template v-if="column.key === 'customer'">
              ...
              <span class="table-subtitle">{{
                record.opportunityCode || "客户日常跟进"
              }}</span>
            </template>
            <template v-else-if="column.key === 'type'"
              ><a-tag :color="followUpTypeColor(record.type)">{{
                followUpTypeLabel(record.type)
              }}</a-tag></template
            >
            ...
            <template v-else-if="column.key === 'followedAt'">{{
              formatDateTime(record.followedAt)
            }}</template>
            <template v-else-if="column.key === 'nextAction'">
              <span :class="{ 'text-danger': isOverdue(record.nextAction) }">{{
                record.nextAction || "-"
              }}</span>
            ...
            <template v-else-if="column.key === 'action'">
              <span @click.stop>
                <a-popconfirm
                  v-if="auth.can('crm:followup:delete')"
                  title="确实删除此跟进记录？"
        ...
        <a-row :gutter="16">
          <a-col :xs="24" :md="12"
            ><a-form-item label="客户" name="customerId"
              ><a-select
                ...
                @change="syncOpportunity" /></a-form-item
          ></a-col>
          <a-col :xs="24" :md="12"
            ><a-form-item label="关联商机"
              ><a-select
                ...
                show-search
                option-filter-prop="label"
                :options="availableOpportunityOptions" /></a-form-item
          ></a-col>
          <a-col :xs="24" :md="8"
          ...
          <a-col :xs="24" :md="8"
            ><a-form-item label="发生时间" name="followedAt"
              ><a-input
                v-model:value="form.followedAt"
                type="datetime-local" /></a-form-item
          ></a-col>
          <a-col :span="24"
            ><a-form-item label="主题" name="subject"
              ><a-input v-model:value="form.subject" /></a-form-item
          ></a-col>
          <a-col :span="24"
          ...
          <a-col :span="24"
            ><a-form-item label="下一步动作"
              ><a-input v-model:value="form.nextAction" /></a-form-item
          ></a-col>
        </a-row>
      </a-form>
    </a-modal>
...
const availableOpportunityOptions = computed(() =>
  opportunities.value
    ...
    .map((o) => ({
      label: `${o.code} ${o.needSummary?.slice(0, 20)}`,
      value: o.id,
    })),
);
...
const pendingGroups = computed(() => {
  const map = new Map<
    string,
    { customerName: string; opportunityCode: string; items: FollowUp[] }
  >();
