<template>
  <div class="page-stack">
    <a-card>
      ...
      <template v-else-if="record">
        <div class="detail-title">
          ...
          <a-progress
            v-if="record.stage !== 'WON' && record.stage !== 'LOST'"
        ...
        >
          ...
          <a-descriptions-item label="成功率"
            >{{ record.probability }}%</a-descriptions-item
          >
          ...
          <a-row :gutter="16">
            ...
            <a-col :span="8">
              <span class="table-subtitle"
                >计划日期：{{ record.nextActionAt || "未设置" }}</span
              >
              <span v-if="actionOverdue" class="table-subtitle text-danger"
                >已逾期</span
              >
            </a-col>
          </a-row>
        ...
        >
          <a-descriptions bordered :column="1">
            <a-descriptions-item label="丢单时间">{{
              formatDateTime(lostFollowUp.followedAt)
            }}</a-descriptions-item>
            <a-descriptions-item label="丢单原因">{{
              lostFollowUp.content.replace("丢单原因：", "")
            }}</a-descriptions-item>
          </a-descriptions>
        </a-card>
        <a-card v-if="relatedQuote" title="关联报价" style="margin-top: 16px">
          <a-button
            ...
            @click="router.push('/crm/quotes/' + relatedQuote.id)"
          >
            {{ relatedQuote.code }} V{{ relatedQuote.versionNo }} ·
            {{ formatMoney(relatedQuote.amount) }}
          </a-button>
          <p style="margin: 4px 0 0; color: #8c8c8c; font-size: 12px">
            状态：<a-tag :color="quoteStatusColor(relatedQuote.status)">{{
            ...
            <template v-if="relatedQuote.convertedContractId">
              ...
              <a
                @click="
                  router.push(
                    '/crm/contracts/' + relatedQuote.convertedContractId,
                  )
                "
                >合同</a
              >
        ...
        <a-card title="跟进记录" style="margin-top: 16px">
          ...
          <a-empty
            v-if="!loading && followUps.length === 0"
            description="暂无跟进记录"
            style="margin: 24px 0"
          />
          ...
          <a-comment
            ...
            :datetime="formatDateTime(item.followedAt)"
          >
            <template #avatar>
              <a-avatar
                :style="{
                  backgroundColor:
                    followUpTypeColor(item.type) === 'red'
                      ? '#f5222d'
                      : followUpTypeColor(item.type) === 'blue'
                        ? '#1890ff'
                        : '#8c8c8c',
                }"
                size="small"
    ...
    <a-modal
      v-model:open="createFollowUpOpen"
      title="新增跟进记录"
      width="640px"
      :confirm-loading="savingFollowUp"
      @ok="handleCreateFollowUp"
    >
      <a-form
        ref="followUpFormRef"
        :model="followUpForm"
        ...
        <a-row :gutter="16">
          ...
          <a-col :xs="24" :md="12">
            <a-form-item label="负责人" name="ownerName">
              <a-select
                v-model:value="followUpForm.ownerName"
                :options="userOptions"
                show-search
          ...
          <a-col :xs="24" :md="12">
            <a-form-item label="发生时间" name="followedAt">
              <a-input
                v-model:value="followUpForm.followedAt"
                type="datetime-local"
              />
            </a-form-item>
          </a-col>
...
const lostFollowUp = computed(() =>
  followUps.value.find((item) => item.subject.startsWith("商机推进至「丢单」")),
);

const stageAdvancements = computed(() =>
  followUps.value.filter((item) => item.subject.startsWith("商机推进至")),
);
...
const timelineEvents = computed(() => {
  ...
  if (relatedQuote.value) {
    events.push({
      ...
      time: formatDateTime(
        relatedQuote.value.createdAt || relatedQuote.value.updatedAt,
      ),
      title:
        "报价创建：" +
        relatedQuote.value.code +
        " V" +
        (relatedQuote.value.versionNo || 1),
      desc:
        "金额 " +
        formatMoney(relatedQuote.value.amount) +
        " · " +
        (relatedQuote.value.serviceScope?.slice(0, 50) || ""),
      color: "purple",
      sortKey:
        relatedQuote.value.createdAt || relatedQuote.value.updatedAt || "",
      isAdvance: false,
