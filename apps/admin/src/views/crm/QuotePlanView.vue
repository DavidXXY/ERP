<template>
  <div class="page-stack quote-page">
    <a-card title="报价方案">
      <template #extra>
        <a-space>
          ...
          <a-button
            v-if="auth.can('crm:quote:create')"
            type="primary"
            @click="openCreate()"
            >新增报价</a-button
          >
      ...
      <a-row :gutter="[16, 16]" class="metric-row">
        <a-col :xs="12" :lg="6"
          ><a-statistic title="报价总数" :value="quotes.length" suffix="份"
        /></a-col>
        <a-col :xs="12" :lg="6"
          ><a-statistic
            title="当前报价总额"
        ...
        <a-col :xs="12" :lg="6"
          ><a-statistic
            ...
            suffix="份"
            :value-style="{
              color: lowMarginCount > 0 ? '#ff4d4f' : '#52c41a',
            }"
        /></a-col>
      </a-row>
      <a-alert
        ...
        type="info"
        show-icon
        message="报价利润校验：销售在报价阶段发起售前支持，项目管理填写并审批成本；成本核对通过后，报价才可以继续提交审批。"
      />
            ...
            (record: any) => ({
              onClick: (e: any) => {
                if (
                  ...
                  return;
                router.push('/crm/quotes/' + record.id);
              },
            })
          "
        >
          <template #bodyCell="{ column, record }">
            ...
            <template v-else-if="column.key === 'scope'">
              ...
              <span v-if="record.inspectCycle" class="table-subtitle">{{
                record.inspectCycle
              }}</span>
            </template>
            ...
            <template v-else-if="column.key === 'margin'">
              <a-tag
                :color="
                  quoteMargin(record).rate < 15
                    ? 'red'
                    : quoteMargin(record).rate < 25
                      ? 'orange'
                      : 'green'
                "
                >{{ quoteMargin(record).rate.toFixed(1) }}%</a-tag
              >
              <span class="table-subtitle"
                >预算 {{ formatMoney(quoteMargin(record).cost) }} · 毛利
                {{ formatMoney(quoteMargin(record).gross) }}</span
              >
            </template>
            ...
            <template v-else-if="column.key === 'customerResult'">
              <template v-if="record.customerDecision">
                ...
                <span class="table-subtitle"
                  >{{ record.customerDecisionBy }} ·
                  {{ formatDateTime(record.customerDecidedAt) }}</span
                >
              </template>
              ...
              <a-space size="small" wrap>
                ...
                >
                  <a-button
                    size="small"
                    type="link"
                    :disabled="!quoteBudgetConfirmed(record)"
                    >提交审批</a-button
                  >
                </a-popconfirm>
                <a-button
                ...
                <a-button
                  v-if="
                    auth.can('crm:quote:customer-result') &&
                ...
                <a-button
                  v-if="
                    auth.can('crm:quote:convert') &&
                    record.status === 'CUSTOMER_ACCEPTED'
                  "
                  size="small"
                  type="link"
      ...
      <div class="mobile-only">
        <div
          v-for="record in filteredQuotes"
          :key="record.id"
          class="mobile-card-item"
          @click="router.push('/crm/quotes/' + record.id)"
        >
          ...
          <div class="mobile-card-tags">
            {{ record.opportunityCode || "未关联商机" }} · 销售负责人
    ...
    <a-modal
      ...
      :title="
        editingQuote
          ? `修订报价 · ${editingQuote.code} V${editingQuote.versionNo}`
          : '新增报价方案'
      "
      width="780px"
    ...
    >
      ...
      <a-form ref="formRef" :model="form" :rules="rules" layout="vertical">
        <a-row :gutter="16">
          ...
          <a-col :span="24"
            ><a-form-item label="服务范围" name="serviceScope"
              ><a-textarea
                v-model:value="form.serviceScope"
                :rows="3" /></a-form-item
          ></a-col>
          <a-col :xs="24" :md="12"
            ><a-form-item label="服务频次"
              ><a-input
                v-model:value="form.inspectCycle"
          ...
          <a-col :span="24">
            <div class="quote-budget-panel">
                    ...
                    :color="
                      formMargin.rate < 15
                        ...
                        : formMargin.rate < 25
                          ...
                          : 'green'
                    "
                    >毛利率 {{ formMargin.rate.toFixed(1) }}%</a-tag
                  >
                  <a-tag v-if="quoteBudgetLocked" color="green"
                    >售前成本已填写，预算只读</a-tag
                  >
                  <a-button
                    size="small"
                    :disabled="quoteBudgetLocked"
                    @click="applyDefaultBudget"
                    >按标准比例生成</a-button
                  >
                </a-space>
              </div>
              ...
              <div
                v-if="
                  editingQuote?.costRequest &&
                  hasFilledPresalesCost(editingQuote.costRequest)
                "
                class="quote-presales-cost"
              >
                <div class="quote-presales-cost-title">
                  <strong>售前支持成本</strong>
                  <span>{{
                    editingQuote.costRequest.projectManager || "未填写负责人"
                  }}</span>
                </div>
                <div class="presales-cost-table">
                  ...
                  <div
                    v-for="item in presalesCostRows(editingQuote.costRequest)"
                    :key="item.key"
                    class="presales-cost-row"
                  >
          ...
          <a-col :span="24"
            ><a-form-item label="付款方式/节点" name="paymentNodes"
              ><a-input
                v-model:value="form.paymentNodes"
                placeholder="例如：签约30%，半年节点30%，验收40%" /></a-form-item
          ></a-col>
          <a-col v-if="editingQuote" :span="24">
            <a-form-item label="本次修订说明" required>
              <a-textarea
                v-model:value="form.revisionNote"
                :rows="2"
                placeholder="说明本次价格、范围、频次或付款节点的调整"
              />
            </a-form-item>
          </a-col>
          <a-col :span="24"
            ><a-form-item :label="editingQuote ? '修订人' : '创建人'"
              ><a-input v-model:value="form.editorName" /></a-form-item
          ></a-col>
        </a-row>
      </a-form>
    </a-modal>
    ...
    <a-modal
      v-model:open="customerResultOpen"
      title="登记客户结果"
      width="640px"
      :confirm-loading="saving"
      @ok="handleCustomerResult"
    >
      <a-alert
        v-if="selectedQuote"
        class="section-alert"
      ...
      />
      <a-form
        ref="customerResultFormRef"
        :model="customerResultForm"
        :rules="customerResultRules"
        layout="vertical"
      >
        <a-form-item label="客户结果" name="decision">
          <a-radio-group
            v-model:value="customerResultForm.decision"
            button-style="solid"
            @change="syncCustomerComment"
          >
            <a-radio-button value="ACCEPTED">客户接受</a-radio-button>
            <a-radio-button value="DECLINED">客户拒绝</a-radio-button>
          </a-radio-group>
        </a-form-item>
        <a-form-item label="登记人" name="operatorName"
          ><a-input v-model:value="customerResultForm.operatorName"
        /></a-form-item>
        <a-form-item label="结果说明" name="comment"
          ><a-textarea v-model:value="customerResultForm.comment" :rows="3"
        /></a-form-item>
      </a-form>
    </a-modal>

    <a-modal
      v-model:open="conversionOpen"
      title="报价转合同"
      width="860px"
      :confirm-loading="saving"
      @ok="handleConversion"
      ...
      >
        ...
        <a-row :gutter="16">
          <a-col :xs="24" :md="8"
            ><a-form-item label="合同编号" name="contractCode"
              ><a-input
                v-model:value="conversionForm.contractCode" /></a-form-item
          ></a-col>
          <a-col :xs="24" :md="10"
            ><a-form-item label="合同项目名称" name="projectName"
              ><a-input
                v-model:value="conversionForm.projectName" /></a-form-item
          ></a-col>
          <a-col :xs="24" :md="6"
            ><a-form-item label="合同类型" name="contractType"
              ><a-select
                v-model:value="conversionForm.contractType"
                :options="contractTypeOptions" /></a-form-item
          ></a-col>
          <a-col :xs="24" :md="8"
            ><a-form-item label="开始日期" name="startDate"
              ><a-input
                v-model:value="conversionForm.startDate"
                type="date" /></a-form-item
          ></a-col>
          <a-col :xs="24" :md="8"
            ><a-form-item label="结束日期" name="endDate"
              ><a-input
                v-model:value="conversionForm.endDate"
                type="date" /></a-form-item
          ></a-col>
          <a-col :xs="24" :md="8"
            ><a-form-item label="服务周期"
              ><a-input
                v-model:value="conversionForm.serviceCycle" /></a-form-item
          ></a-col>
          <a-col :span="24"
            ><a-form-item label="付款方式/节点" required
              ><a-input
                :value="selectedQuote?.paymentNodes || ''"
                disabled
                placeholder="请先在报价中填写付款方式/节点" /></a-form-item
          ></a-col>
        </a-row>
        ...
        <a-form-item label="合同审批件" required>
          <a-upload
            ...
            :before-upload="selectConversionAttachment"
            accept=".jpg,.jpeg,.png,.webp,.pdf,.doc,.docx,.xls,.xlsx"
          >
            <a-button
              ><template #icon><UploadOutlined /></template>选择审批件</a-button
            >
          </a-upload>
          <div v-if="conversionAttachment" class="selected-attachment">
            ...
            <a-button
              type="link"
              size="small"
              @click="conversionAttachment = null"
              >移除</a-button
            >
          </div>
          <div v-else class="form-help">
            请上传合同审批件，转合同后将自动归档到合同审批件。
          </div>
        </a-form-item>
    ...
    <a-modal
      v-model:open="costRequestOpen"
      title="发起售前支持"
      width="520px"
      :confirm-loading="saving"
      @ok="handleCostRequest"
    >
      <a-alert
        v-if="selectedQuote"
        class="section-alert"
      ...
      />
      <a-form
        ref="costRequestFormRef"
        :model="costRequestForm"
        :rules="costRequestRules"
        layout="vertical"
      >
        <a-form-item label="询价发起人" name="requestedBy"
          ><a-input v-model:value="costRequestForm.requestedBy"
        /></a-form-item>
      </a-form>
    </a-modal>

    <a-modal
      v-model:open="costSubmitOpen"
      title="填写售前成本"
      width="780px"
      :confirm-loading="saving"
      @ok="handleCostSubmit"
    >
      ...
      <a-form
        ...
        :model="costSubmitForm"
        :rules="costSubmitRules"
        layout="vertical"
      >
        <a-row :gutter="12">
          <a-col :xs="24" :md="8">
            <a-form-item label="成本负责人" name="projectManager">
              <a-select
                v-model:value="costSubmitForm.projectManager"
                :options="userOptions"
                show-search
                option-filter-prop="label"
                placeholder="选择成本负责人"
          ...
          ></a-col>
          <a-col :xs="12" :md="8"
            ><a-form-item label="风险预留"
              ><a-input-number
                v-model:value="costSubmitForm.riskReserve"
                :min="0"
                :precision="2"
                class="full-input" /></a-form-item
          ></a-col>
          ...
          <a-col :span="24"
            ><a-form-item label="成本说明"
              ><a-textarea
                v-model:value="costSubmitForm.costRemark"
                :rows="3" /></a-form-item
          ></a-col>
        </a-row>
      </a-form>
    ...
    <a-modal
      v-model:open="costApprovalOpen"
      title="售前成本审批"
      width="620px"
      :confirm-loading="saving"
      @ok="handleCostApproval"
    >
      <a-alert
      ...
      >
        <ApprovalProgressFlow
          :steps="quoteCostApprovalSteps(selectedCostRequest)"
        />
      </a-card>
      <a-form
        ref="costApprovalFormRef"
        :model="costApprovalForm"
        :rules="costApprovalRules"
        layout="vertical"
      >
        <a-form-item label="审批结论" name="decision">
          <a-radio-group
            v-model:value="costApprovalForm.decision"
            button-style="solid"
          >
            <a-radio-button value="APPROVED">通过</a-radio-button>
            <a-radio-button value="REJECTED">驳回</a-radio-button>
        ...
        <a-form-item label="审批人" name="approverName"
          ><a-input v-model:value="costApprovalForm.approverName"
        /></a-form-item>
        <a-form-item label="审批意见" name="comment"
          ><a-textarea v-model:value="costApprovalForm.comment" :rows="3"
        /></a-form-item>
      </a-form>
    </a-modal>
      ...
      >
        <template #bodyCell="{ column, record }">
          <template v-if="column.key === 'version'"
            ><strong>V{{ record.versionNo }}</strong></template
          >
          <template v-else-if="column.key === 'content'">
            <span class="line-clamp-2">{{ record.serviceScope }}</span>
            <span class="table-subtitle"
              >{{ record.inspectCycle || "未设置频次" }} ·
              {{ formatMoney(record.amount) }} · 毛利率
              {{ quoteMargin(record).rate.toFixed(1) }}%</span
            >
          </template>
          <template v-else-if="column.key === 'revision'">
            ...
            <span class="table-subtitle"
              >{{ record.editorName }} ·
              {{ formatDateTime(record.revisedAt) }}</span
            >
          </template>
        </template>
      </a-table>
...
const receivableAllocated = computed(() =>
  (conversionForm.receivables || []).reduce(
...
const opportunityOptions = computed(() =>
  opportunities.value
    ...
    .map((item) => ({
      label: `${item.code} · ${item.customerName} · ${item.needSummary}`,
      value: item.id,
    })),
);
  ...
  return quotes.value
    ...
    .sort((a, b) => {
      const convertedOrder =
        Number(a.status === "CONVERTED") - Number(b.status === "CONVERTED");
      if (convertedOrder !== 0) return convertedOrder;
      return new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime();
    });
});
const totalAmount = computed(() =>
  quotes.value.reduce((sum, item) => sum + Number(item.amount || 0), 0),
);
const customerPendingCount = computed(
  () => quotes.value.filter((item) => item.status === "APPROVED").length,
);
const customerAcceptedCount = computed(
  () =>
    quotes.value.filter((item) => item.status === "CUSTOMER_ACCEPTED").length,
);
const lowMarginCount = computed(
  () => quotes.value.filter((item) => quoteMargin(item).rate < 15).length,
);
const selectedQuoteMargin = computed(() =>
  selectedQuote.value
    ? quoteMargin(selectedQuote.value)
    : { cost: 0, gross: 0, rate: 0 },
);
const quoteBudgetLocked = computed(() =>
  editingQuote.value?.costRequest
    ? hasFilledPresalesCost(editingQuote.value.costRequest)
    : false,
);
...
function quoteOwnerName(
  record: Pick<QuotePlan, "opportunityId" | "opportunityCode">,
) {
...
function quoteMargin(
  ...
  const rate = Number(
    record.grossMarginRate ?? (amount > 0 ? (gross / amount) * 100 : 0),
  );
  return { cost, gross, rate };
}

function quoteBudgetTotal(record: {
  laborBudget?: number;
...
function quoteBudgetRawTotal(record: {
  laborBudget?: number;
  materialBudget?: number;
  subcontractBudget?: number;
  travelBudget?: number;
...
function quoteCostTotal(record: {
  laborCost?: number;
  materialCost?: number;
...
function presalesCostRows(cost: QuoteCostRequest) {
  return [
    ...
    presalesCostRow(
      ...
      "材料",
      cost.materialCost,
      cost.materialTaxRate ?? 13,
    ),
    presalesCostRow(
      "subcontract",
      "外包",
      cost.subcontractCost,
      cost.subcontractTaxRate ?? 13,
    ),
...
onMounted(async () => {
  await loadData();
  const opportunityId =
    typeof route.query.opportunity === "string"
...
async function handleSaveQuote() {
  ...
  try {
    if (editingQuote.value) {
      ...
      message.success(
        `已生成 ${editingQuote.value.code} V${editingQuote.value.versionNo + 1}`,
      );
    } else {
      await createQuote({
...
function canApproveCost(record: QuotePlan) {
  return (
    record.status === "COSTING" && record.costRequest?.status === "SUBMITTED"
  );
}
...
async function handleCostApproval() {
  ...
  saving.value = true;
  try {
    await approveQuoteCost(selectedCostRequest.value.id, {
      ...costApprovalForm,
    });
    costApprovalOpen.value = false;
    message.success(
      costApprovalForm.decision === "APPROVED"
        ? "成本已核对，销售可以提交报价审批"
        : "成本已驳回，可重新填写",
    );
    await loadData();
...
function quoteBudgetConfirmed(record: QuotePlan) {
  return (
    record.status === "COST_APPROVED" &&
    record.costRequest?.status === "APPROVED"
  );
}
...
function quoteCostApprovalSteps(
  item: QuoteCostRequest,
): ApprovalProgressStep[] {
  return [
    {
      key: "request",
      personName: item.requestedBy || "发起人",
      title: "发起成本核算",
      time: item.requestedAt,
      note: selectedQuote.value?.code || selectedQuote.value?.customerName,
      state: "done",
    },
    {
      key: "submit",
      personName: item.projectManager || "成本负责人",
      title: item.status === "REQUESTED" ? "待填写" : "已提交",
      time: item.submittedAt,
      note: item.costRemark || "成本测算",
      state: item.status === "REQUESTED" ? "pending" : "done",
    },
    {
      key: "approval",
      ...
      note:
        item.status === "SUBMITTED"
          ? "等待售前成本审批"
          : item.approvalComment || "审批后销售可提交报价",
      state:
        item.status === "APPROVED"
          ? "done"
          : item.status === "REJECTED"
...
async function handleApproval() {
  ...
  saving.value = true;
  try {
    const result = await processQuoteApproval(selectedQuote.value.id, {
      ...approvalForm,
    });
...
function syncCustomerComment() {
  customerResultForm.comment =
    customerResultForm.decision === "ACCEPTED"
      ? "客户确认接受本版报价，进入合同签署准备。"
      : "客户未接受本版报价，记录原因后继续修订。";
}
...
async function handleCustomerResult() {
  ...
  saving.value = true;
  try {
    const result = await processQuoteCustomerResult(selectedQuote.value.id, {
      ...customerResultForm,
    });
    Object.assign(selectedQuote.value, result);
    customerResultOpen.value = false;
    message.success(
      result.status === "CUSTOMER_ACCEPTED"
        ? "客户已接受，可继续转合同"
        : "已登记客户拒绝，可继续修订报价",
    );
    await loadData();
...
async function handleConversion() {
  ...
  saving.value = true;
  try {
    const result = await convertQuote(selectedQuote.value.id, {
      ...conversionForm,
    });
    ...
    try {
      await uploadAttachment(
        ...
        result.contract.id,
        "APPROVAL_DOC",
        conversionAttachment.value,
      );
    } catch (error) {
      conversionOpen.value = false;
      message.warning(
        `合同已生成，但附件上传失败：${error instanceof Error ? error.message : "请到合同详情页补传"}`,
      );
      await loadData();
      return;
    }
...
function initialCostSubmitForm(cost?: QuoteCostRequest) {
  return {
    ...
    suggestedPrice: Number(
      cost?.suggestedPrice || selectedQuote.value?.amount || 0,
    ),
    ...
    receivables: [
      {
        rowKey: "r1",
        receivableCode: "",
        amount: Math.round((Number(record?.amount || 0) * 50) / 100),
        ratio: 50,
        dueDate: formatDate(new Date(Date.now() + 30 * 86400000)),
      },
      {
        rowKey: "r2",
        receivableCode: "",
        amount:
          ...
          Math.round((Number(record?.amount || 0) * 50) / 100),
        ratio: 50,
        dueDate: formatDate(new Date(Date.now() + 90 * 86400000)),
      },
    ],
  };
}
