  ...
  <div class="page-stack">
    <a-card title="续约管理">
      ...
      <a-row :gutter="[16, 16]" class="metric-row">
        <a-col :xs="12" :lg="6"
          ><a-statistic title="续约清单" :value="renewals.length" suffix="份"
        /></a-col>
        <a-col :xs="12" :lg="6"
          ><a-statistic title="30天内到期" :value="dueSoonCount" suffix="份"
        /></a-col>
        <a-col :xs="12" :lg="6"
          ><a-statistic title="已到期" :value="expiredCount" suffix="份"
        /></a-col>
        <a-col :xs="12" :lg="6"
          ><a-statistic
            title="关联待收"
      ...
      <a-table
        :columns="columns"
        :data-source="filteredRenewals"
        :loading="loading"
        :row-key="(record: Renewal) => record.contractId"
        :scroll="{ x: 1220 }"
      >
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.key === 'endDate'">
            ...
            <span
              ...
              :class="{ 'text-danger': record.daysRemaining <= 30 }"
              >{{ daysLabel(record.daysRemaining) }}</span
            >
          </template>
          <template v-else-if="column.key === 'risk'"
            ><a-tag :color="riskColor(record.renewalRisk)">{{
              riskLabel(record.renewalRisk)
            }}</a-tag></template
          >
          <template v-else-if="column.key === 'outstanding'"
            ><span :class="{ 'text-danger': record.outstandingAmount > 0 }">{{
...
const dueSoonCount = computed(
  () =>
    renewals.value.filter(
      (item) => item.daysRemaining >= 0 && item.daysRemaining <= 30,
    ).length,
);
const expiredCount = computed(
  () => renewals.value.filter((item) => item.daysRemaining < 0).length,
);
const outstandingAmount = computed(() =>
  renewals.value.reduce(
...
function riskLabel(risk: Renewal["renewalRisk"]) {
  return { EXPIRED: "已到期", HIGH: "高风险", MEDIUM: "需跟进", LOW: "低风险" }[
    risk
  ];
}
...
function viewCustomer(record: Renewal) {
  router.push({
    path: "/crm/customers",
    query: { customer: record.customerId },
  });
}
