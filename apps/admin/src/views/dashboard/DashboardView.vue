<template>
  <div class="page-stack">
    <a-card title="经营驾驶舱"
      ><template #extra
        ><a-space
          ><a-range-picker v-model:value="range" @change="loadData" /><a-button
            :loading="loading"
            @click="loadData"
      ...
      <a-row :gutter="[16, 16]" class="metric-row"
        ...
        ><a-col :xs="12" :xl="6"
          ><a-statistic
            title="毛利率"
            :value="company.grossMarginRate"
            suffix="%"
            :value-style="valueStyle(company.grossProfit)" /></a-col
        ><a-col :xs="12" :xl="6"
          ><a-statistic
            title="预算偏差率"
            :value="company.projectBudgetDeviationRate"
            suffix="%"
            :value-style="
              dangerStyle(Math.abs(company.projectBudgetDeviationRate))
            " /></a-col
        ><a-col :xs="12" :xl="6"
          ><a-statistic
            title="库存周转"
            :value="company.inventoryTurnover"
            suffix="次" /></a-col
        ><a-col :xs="12" :xl="6"
          ><a-statistic
            title="采购占比"
            :value="company.procurementRatio"
            suffix="%" /></a-col
        ><a-col :xs="12" :xl="6"
          ><a-statistic
            title="现金净流"
      ...
      <a-row :gutter="[16, 16]" class="metric-row"
        ...
        ><a-col :xs="12" :xl="6"
          ><a-statistic
            title="待续约合同"
            :value="data.summary.renewalRisks"
            suffix="份"
            :value-style="dangerStyle(data.summary.renewalRisks)" /></a-col
      ></a-row>
      <a-descriptions bordered :column="4" size="small"
        ...
        ><a-descriptions-item label="低库存物料"
          ><span :class="{ 'text-danger': data.summary.lowStockParts > 0 }">{{
            data.summary.lowStockParts
          }}</span></a-descriptions-item
        ><a-descriptions-item label="客户投诉"
          ><span :class="{ 'text-danger': data.summary.complaints > 0 }">{{
    ...
    <a-card title="近六月经营趋势"
      ><a-table
        size="small"
        :columns="trendColumns"
        :data-source="data.monthlyTrends"
        :pagination="false"
        :row-key="(item: MonthlyTrend) => item.month"
        ><template #bodyCell="{ column, record }"
          ...
          ><template v-else-if="column.key === 'cash'"
            >收入 {{ formatMoney(record.cashIn)
            }}<span class="table-subtitle"
              >支出 {{ formatMoney(record.cashOut) }}</span
            ></template
          ><template v-else-if="column.key === 'profit'"
            ><strong :class="{ 'text-danger': record.profit < 0 }">{{
              formatMoney(record.profit)
            }}</strong></template
          ><template v-else-if="column.key === 'trend'"
            ><a-progress
              :percent="trendPercent(record.revenue)"
              :show-info="false"
              size="small" /></template></template></a-table
    ></a-card>

    <a-card
      ><a-tabs
        ><a-tab-pane key="customers" tab="客户利润"
          ><a-table
            :columns="customerColumns"
            :data-source="data.customerProfits"
            :pagination="{ pageSize: 8 }"
            :row-key="(item: CustomerProfit) => item.customerId"
            :scroll="{ x: 1100 }"
            ><template #bodyCell="{ column, record }"
              ><template v-if="column.key === 'contract'"
                >{{ formatMoney(record.contractAmount)
                }}<span class="table-subtitle"
                  >已回款 {{ formatMoney(record.receivedAmount) }}</span
                ></template
              ><template v-else-if="column.key === 'workorder'"
                >收入 {{ formatMoney(record.workOrderRevenue)
                }}<span class="table-subtitle"
                  >成本 {{ formatMoney(record.workOrderCost) }}</span
                ></template
              ><template v-else-if="column.key === 'profit'"
                ><strong :class="{ 'text-danger': record.grossProfit < 0 }">{{
                  formatMoney(record.grossProfit)
                }}</strong
                ><span class="table-subtitle"
                  >毛利率 {{ record.margin }}%</span
                ></template
              ><template v-else-if="column.key === 'complaints'"
                ><a-tag :color="record.complaints ? 'red' : 'green'">{{
                  record.complaints
                }}</a-tag></template
              ></template
            ></a-table
          ></a-tab-pane
        >
        <a-tab-pane key="equipment" tab="设备故障"
          ><a-table
            :columns="equipmentColumns"
            :data-source="data.equipmentPerformance"
            :pagination="{ pageSize: 8 }"
            :row-key="(item: EquipmentPerformance) => item.equipmentId"
            ><template #bodyCell="{ column, record }"
              ><template v-if="column.key === 'equipment'"
                ><strong>{{ record.equipmentCode }}</strong
                ><span class="table-subtitle">{{
                  ...
                  :color="
                    record.faultCount > 2
                      ...
                      : record.faultCount
                        ...
                        : 'green'
                  "
                  >{{ record.faultCount }} 次</a-tag
                ></template
              ><template v-else-if="column.key === 'cost'"
                >{{ formatMoney(record.maintenanceCost)
                }}<span class="table-subtitle"
                  >单次 {{ formatMoney(record.averageCost) }}</span
                ></template
              ></template
            ></a-table
          ></a-tab-pane
        >
        <a-tab-pane key="workforce" tab="人员产值"
          ><a-table
            :columns="workforceColumns"
            :data-source="data.workforcePerformance"
            :pagination="{ pageSize: 8 }"
            :row-key="(item: WorkforcePerformance) => item.userId"
            ><template #bodyCell="{ column, record }"
              ><template v-if="column.key === 'hours'"
                >{{ record.laborHours }} 小时</template
              ><template v-else-if="column.key === 'revenue'"
                >{{ formatMoney(record.generatedRevenue)
                }}<span class="table-subtitle"
                  >人工成本 {{ formatMoney(record.laborCost) }}</span
                ></template
              ><template v-else-if="column.key === 'output'"
                >{{ formatMoney(record.outputPerHour) }} / 小时</template
              ></template
            ></a-table
          ></a-tab-pane
        ></a-tabs
...
import {
  getCompanyKpiDashboard,
  getExecutiveDashboard,
  type CompanyKpiDashboard,
  type CustomerProfit,
  type EquipmentPerformance,
  type ExecutiveDashboard,
  type MonthlyTrend,
  type WorkforcePerformance,
} from "@/api/bi";
const loading = ref(false);
const data = reactive<ExecutiveDashboard>(empty());
const company = reactive<CompanyKpiDashboard>(emptyCompany());
const range = ref<any>();
const maxRevenue = computed(() =>
  Math.max(...data.monthlyTrends.map((i) => i.revenue), 1),
);
const trendColumns = [
...
const equipmentColumns = [
  { title: "设备", key: "equipment", width: 250 },
...
async function loadData() {
  loading.value = true;
  try {
    const params = range.value?.length
      ? {
    ...
    Object.assign(company, kpi);
  } catch (error) {
    message.error(error instanceof Error ? error.message : "经营数据加载失败");
  } finally {
    loading.value = false;
  }
}
function empty(): ExecutiveDashboard {
  return {
    summary: {
      contractRevenue: 0,
      receivedAmount: 0,
...
function emptyCompany(): CompanyKpiDashboard {
  return {
    startDate: "",
    endDate: "",
    revenue: 0,
    ...
    projectBudget: 0,
    projectActualCost: 0,
    projectBudgetDeviationRate: 0,
    procurementAmount: 0,
    procurementRatio: 0,
    inventoryValue: 0,
    inventoryTurnover: 0,
    workforceOutputPerHour: 0,
    trends: [],
    topCustomers: [],
  };
}
function trendPercent(v: number) {
...
function dangerStyle(v: number) {
  return v > 0 ? { color: "#cf1322" } : {};
}
function valueStyle(v: number) {
  return v < 0 ? { color: "#cf1322" } : { color: "#237804" };
}
</script>
