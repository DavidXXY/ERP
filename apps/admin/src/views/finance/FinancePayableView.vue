<template>
  <div class="page-stack">
    <a-card title="应付管理">
      ...
      <section class="finance-action-panel">
        ...
        <div class="schedule-grid">
          <div v-for="bucket in scheduleBuckets" :key="bucket.key" class="schedule-card">
          ...
          >
            ...
            <strong>{{ formatMoney(item.availableAmount || item.outstandingAmount) }}</strong>
      ...
      <a-space wrap class="table-toolbar">
        ...
        <a-select v-model:value="scheduleFilter" allow-clear placeholder="付款排期" :options="scheduleOptions" style="width: 150px" />
      </a-space>
      <a-table :columns="columns" :data-source="filteredItems" :loading="loading" :pagination="{ pageSize: 8 }" :row-key="(item: FinancePayable) => item.id" :scroll="{ x: 1280 }">
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.key === 'schedule'">
            <a-tag :color="scheduleColor(record)">{{ scheduleLabel(record) }}</a-tag>
    ...
    <a-modal v-model:open="applicationOpen" title="新增付款申请" width="720px" :confirm-loading="saving" @ok="handleCreateApplication">
      ...
      <a-alert v-if="selectedItem?.overdue" class="section-alert" type="warning" show-icon :message="`该应付已${dueLabel(selectedItem)}，建议优先处理或说明延付原因。`" />
      <a-alert v-if="selectedItem && selectedItem.reservedAmount > 0" class="section-alert" type="info" show-icon :message="`已有审批占用 ${formatMoney(selectedItem.reservedAmount)}，本次申请不得超过剩余可申请金额。`" />
      <a-form ref="formRef" :model="form" :rules="rules" layout="vertical">
        <div class="quick-amounts">
          <a-button size="small" @click="setApplicationRatio(0.3)">30%</a-button>
          <a-button size="small" @click="setApplicationRatio(0.5)">50%</a-button>
...
const scheduleOptions = [
  { label: "已逾期", value: "OVERDUE" }, { label: "7天内到期", value: "DUE_SOON" }, { label: "已占用", value: "RESERVED" },
];
const columns = [
  ...
  { title: "付款排期", key: "schedule", width: 150 }, { title: "到期日", dataIndex: "dueDate", width: 120 }, { title: "状态", key: "status", width: 160 },
...
const scheduleBuckets = computed(() => [
  ...
  buildScheduleBucket("d7", "7天内到期", (item) => !item.overdue && dueDays(item) <= 7),
  buildScheduleBucket("d30", "30天内到期", (item) => !item.overdue && dueDays(item) <= 30),
  buildScheduleBucket("reserved", "审批占用", (item) => Number(item.reservedAmount || 0) > 0),
]);
const paymentFocus = computed(() => items.value
  .filter((item) => item.status !== "PAID" && item.status !== "CANCELLED" && Number(item.availableAmount || item.outstandingAmount || 0) > 0)
...
function buildScheduleBucket(key: string, label: string, predicate: (item: FinancePayable) => boolean) {
  const rows = items.value.filter((item) => item.status !== "PAID" && item.status !== "CANCELLED" && predicate(item));
  return { key, label, count: rows.length, amount: rows.reduce((sum, item) => sum + Number(item.outstandingAmount || 0), 0) };
}
function showDueSoon() { scheduleFilter.value = "DUE_SOON"; statusFilter.value = undefined; }
function showOverdue() { scheduleFilter.value = "OVERDUE"; statusFilter.value = undefined; }
function exportPayables() {
  const headers = ["应付单", "供应商", "采购单", "应付金额", "已付", "待付", "审批占用", "可申请", "到期日", "状态", "付款排期"];
...
function scheduleScore(item: FinancePayable) {
  ...
  return overdueWeight + dueWeight + reservedWeight + Number(item.availableAmount || item.outstandingAmount || 0);
}
...
function scheduleLabel(item: FinancePayable) { return ({ OVERDUE: "逾期", DUE_SOON: "即将到期", RESERVED: "已占用", NORMAL: "正常" } as Record<string, string>)[scheduleLevel(item)]; }
function scheduleColor(item: FinancePayable) { return ({ OVERDUE: "red", DUE_SOON: "orange", RESERVED: "blue", NORMAL: "green" } as Record<string, string>)[scheduleLevel(item)]; }
