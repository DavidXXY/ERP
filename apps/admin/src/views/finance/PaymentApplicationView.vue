<template>
  <div class="page-stack">
    <a-card title="付款申请">
      ...
      <a-row :gutter="[16, 16]" class="metric-row">
        <a-col :xs="12" :lg="6"
          ><a-statistic
            title="待审批"
            :value="countByStatus('PENDING_APPROVAL')"
            suffix="笔"
        /></a-col>
        <a-col :xs="12" :lg="6"
          ><a-statistic
      ...
      <section class="payment-workbench">
        <div class="workbench-heading">
          ...
          <a-space>
            <a-button size="small" @click="statusFilter = 'PENDING_APPROVAL'"
              >待审批</a-button
            >
            <a-button size="small" @click="statusFilter = 'APPROVED'"
              >待付款</a-button
            >
          </a-space>
        </div>
      ...
      >
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.key === 'risk'">
            ...
            <span class="table-subtitle">{{
              applicationAgeLabel(record)
            }}</span>
          </template>
          <template v-else-if="column.key === 'applicant'">
            {{ record.applicantName }}
            <span class="table-subtitle line-clamp-2">{{
              record.purpose
            }}</span>
          </template>
      ...
      >
        <template #bodyCell="{ column, record }">
          <template v-if="column.key === 'payment'">
            ...
            <span class="table-subtitle"
              >申请单 {{ record.applicationCode }}</span
            >
          </template>
          <template v-else-if="column.key === 'payable'">
            {{ record.payableCode }}
    ...
    >
      ...
      >
        <ApprovalProgressFlow
          :steps="paymentApprovalSteps(selectedApplication)"
        />
      </a-card>
      <a-form
        v-if="canApproveSelected"
        ref="approvalFormRef"
        :model="approvalForm"
        :rules="approvalRules"
        layout="vertical"
    ...
    >
      ...
      <a-alert
        v-if="
          selectedApplication && riskLevel(selectedApplication) !== 'NORMAL'
        "
        class="section-alert"
        type="info"
        show-icon
        :message="`该申请标记为${riskLabel(selectedApplication)}，执行付款前请复核审批意见、供应商和金额。`"
      />
      <a-alert
        v-if="duplicateBankReference"
        class="section-alert"
        type="error"
        show-icon
        message="银行流水 / 付款凭证号已存在，请核对是否重复付款。"
      />
      <a-form
        ref="paymentFormRef"
        :model="paymentForm"
      ...
      >
        <a-row :gutter="16">
          <a-col :xs="24" :md="12"
            ><a-form-item label="付款单号" name="paymentCode"
...
const applicationColumns = [
  ...
  { title: "审批信息", key: "approval", width: 220 },
...
const filteredApplications = computed(() =>
  applications.value.filter((item) => {
    ...
    const text =
      `${item.code} ${item.payableCode} ${item.supplierName} ${item.applicantName} ${item.purpose}`.toLowerCase();
    return (
...
const pendingApprovalAmount = computed(() =>
  applications.value
...
const largeApplications = computed(() =>
  applications.value.filter((item) => riskLevel(item) === "LARGE"),
);
const agedApplications = computed(() =>
...
async function handleApproval() {
  ...
  saving.value = true;
  try {
    await processPaymentApplication(selectedApplication.value.id, {
      ...approvalForm,
    });
...
function countByStatus(status: PaymentApplicationStatus) {
...
function applicationAge(item: PaymentApplication) {
  ...
  return Math.max(
    0,
    Math.floor((todayDate.getTime() - requested.getTime()) / 86400000),
  );
}
function riskLevel(item: PaymentApplication) {
  if (
    Number(item.requestedAmount || 0) >= 100000 &&
    item.status !== "PAID" &&
    item.status !== "REJECTED"
  )
    return "LARGE";
  if (
    applicationAge(item) > 3 &&
    (item.status === "PENDING_APPROVAL" || item.status === "APPROVED")
  )
    return "AGED";
  return "NORMAL";
}
function riskLabel(item: PaymentApplication) {
  return (
    { LARGE: "大额", AGED: "滞留", NORMAL: "正常" } as Record<string, string>
  )[riskLevel(item)];
}
function riskColor(item: PaymentApplication) {
  return (
    { LARGE: "red", AGED: "orange", NORMAL: "green" } as Record<string, string>
  )[riskLevel(item)];
}
...
function statusLabel(status: PaymentApplicationStatus) {
  return (
    {
      ...
      REJECTED: "已驳回",
      PAID: "已付款",
    } as Record<PaymentApplicationStatus, string>
  )[status];
}
function statusColor(status: PaymentApplicationStatus) {
  return (
    {
...
function generateCode(prefix: string) {
  const value = new Date();
  return `${prefix}-${today().replaceAll("-", "")}-${String(value.getHours()).padStart(2, "0")}${String(value.getMinutes()).padStart(2, "0")}${String(value.getSeconds()).padStart(2, "0")}${String(value.getMilliseconds()).padStart(3, "0")}`;
}
  ...
  return [
    ...
    },
    {
      ...
      note:
        item.status === "PENDING_APPROVAL"
          ? "等待付款审批"
          : item.approvalComment || statusLabel(item.status),
      state:
        item.status === "PENDING_APPROVAL"
          ? "pending"
          : item.status === "REJECTED"
    ...
    {
      key: "paid",
      personName: item.paymentCode || "付款经办",
      title: item.status === "PAID" ? "已付款" : "待付款",
      note: approved ? item.paymentCode || "等待付款执行" : "审批通过后执行",
      state: item.status === "PAID" ? "done" : "waiting",
    },
  ];
}
</script>
