<template>
  <div class="page-stack">
      ...
      <a-row :gutter="[16, 16]" class="metric-row">
        ...
        <a-col :xs="12" :xl="4"
          ><a-card
            ><a-statistic
              ...
              suffix="人"
              :value-style="{ color: '#3f8600' }" /></a-card
        ></a-col>
        <a-col :xs="12" :xl="4"
          ><a-card
            ><a-statistic
              title="已离职"
              :value="analytics.leftEmployees"
              suffix="人"
              :value-style="{
                color: analytics.leftEmployees > 0 ? '#ff4d4f' : undefined,
              }" /></a-card
        ></a-col>
        <a-col :xs="12" :xl="4"
          ><a-card
        ...
        <a-col :xs="12" :xl="4"
          ><a-card
            ><a-statistic
              title="待批请假"
              :value="analytics.leavePendingCount"
              suffix="单"
              :value-style="{
                color: analytics.leavePendingCount > 0 ? '#faad14' : undefined,
              }" /></a-card
        ></a-col>
        <a-col :xs="12" :xl="4"
          ><a-card
            ><a-statistic
              ...
              :precision="1"
              :value-style="{
                color: turnoverRate > 10 ? '#ff4d4f' : undefined,
              }" /></a-card
        ></a-col>
      </a-row>
    </a-card>
    ...
    <a-row :gutter="12">
      <a-col :xs="24" :lg="8">
        <a-card title="人员状态" class="analytics-card">
          <div
            v-if="analytics.statusDistribution.length === 0"
            class="chart-empty"
          >
            暂无数据
          </div>
            ...
            >
              ...
              <div class="dist-track">
                <div
                  ...
                  :style="{
                    width:
                      distPercent(item.count, analytics.statusDistribution) +
                      '%',
                    background: statusColor(item.name),
                  }"
                ></div>
              </div>
      ...
      <a-col :xs="24" :lg="8">
        <a-card title="学历分布" class="analytics-card">
          <div
            v-if="analytics.educationDistribution.length === 0"
            class="chart-empty"
          >
            暂无数据
          </div>
          <div v-else class="dist-rows">
            <div
              v-for="item in analytics.educationDistribution"
              :key="item.name"
              class="dist-row"
            >
              <span class="dist-label">{{ item.name }}</span>
              <div class="dist-track">
                <div
                  ...
                  :style="{
                    width:
                      distPercent(item.count, analytics.educationDistribution) +
                      '%',
                    background: '#1890ff',
                  }"
                ></div>
      ...
      <a-col :xs="24" :lg="8">
        <a-card title="组织分布 TOP10" class="analytics-card">
          <div v-if="topOrganizations.length === 0" class="chart-empty">
            暂无数据
          </div>
          <div v-else class="dist-rows">
            <div
              v-for="item in topOrganizations"
              :key="item.name"
              class="dist-row"
            >
              <span class="dist-label">{{ item.name }}</span>
              <div class="dist-track">
                <div
                  class="dist-fill"
                  :style="{
                    width: distPercent(item.count, topOrganizations) + '%',
                    background: '#722ed1',
                  }"
                ></div>
              </div>
    ...
    <a-card title="近期人员变动" class="analytics-card">
      <div
        v-if="analytics.recentLifecycles.length === 0"
...
const topOrganizations = computed(() =>
  [...analytics.value.organizationDistribution]
...
function statusColor(name: string) {
  return (
    { 在职: "#52c41a", 试用: "#1890ff", 离职: "#ff4d4f", 休假: "#faad14" }[
      name
    ] || "#8c8c8c"
  );
}
...
function typeColor(t: string) {
  return (
    ({ 入职: "green", 调岗: "blue", 离职: "red" } as Record<string, string>)[
      t
    ] || "gray"
  );
}
