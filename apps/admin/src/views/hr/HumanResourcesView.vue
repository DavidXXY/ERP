<template>
  <div class="human-resources-page">
    <a-row :gutter="12" class="hr-metrics">
      <a-col :xs="12" :sm="6">
        <div class="metric-card">
          <div class="metric-value">{{ analytics.activeEmployees }}</div>
          <div class="metric-label">在职人数</div>
        </div>
      </a-col>
      <a-col :xs="12" :sm="6">
        <div class="metric-card">
      ...
      <a-col :xs="12" :sm="6">
        <div class="metric-card">
          <div class="metric-value" style="color: #faad14">
            {{ analytics.leavePendingCount }}
          </div>
          <div class="metric-label">待审批请假</div>
        </div>
      </a-col>
      <a-col :xs="12" :sm="6">
        <div class="metric-card">
          <div class="metric-value">{{ analytics.totalEmployees }}</div>
    ...
    <a-space class="hr-action-bar">
      <a-dropdown>
        <a-button
          ><template #icon><DownloadOutlined /></template>导出/导入</a-button
        >
        <template #overlay>
          <a-menu>
            <a-menu-item key="export" @click="exportData"
              >导出员工数据(Excel)</a-menu-item
            >
            <a-menu-item key="import" @click="openImport"
              >导入员工数据</a-menu-item
            >
            <a-menu-item key="template" @click="downloadTemplate"
              >下载导入模板</a-menu-item
            >
          </a-menu>
        </template>
...
async function doImport() {
  ...
  try {
    ...
    const importResult = await importEmployeesExcel(
      importFile.value,
...
const canViewEmployees = computed(() =>
  auth.can("qualification:employee:view"),
);
const canViewCertificates = computed(() =>
  auth.can("qualification:certificate:view"),
);
...
const activeTab = computed<HumanResourcesTab>({
  get() {
    ...
    return requested && availableTabs.value.includes(requested)
      ? requested
      : availableTabs.value[0] || "employees";
  },
  set(value) {
