  ...
  <div class="page-stack">
    <a-card title="请假管理">
      ...
      <a-row :gutter="16" class="metric-row">
        ...
        /></a-col>
        <a-col :xs="12" :lg="6"
          ><a-statistic title="本月请假" :value="monthlyCount"
        /></a-col>
      </a-row>
          ...
          <template v-else-if="column.key === 'actions'">
            <a-space v-if="record.status === 'PENDING'">
              <a-button
                ...
                type="link"
                size="small"
                @click="openApproveLeave(record, true)"
                >通过</a-button
              >
              <a-button
                v-if="canManage"
                ...
                danger
                size="small"
                @click="openApproveLeave(record, false)"
                >驳回</a-button
              >
            </a-space>
          </template>
      ...
      <a-form ref="leaveFormRef" :model="leaveForm" layout="vertical">
        <a-row :gutter="14">
          <a-col :span="12">
            <a-form-item label="员工" name="employeeId" :rules="requiredRule">
              <a-select
                v-model:value="leaveForm.employeeId"
                show-search
          ...
          <a-col :span="12">
            ...
            >
              <a-select
                v-model:value="leaveForm.leaveType"
        ...
        <a-row :gutter="14">
          <a-col :span="8">
            ...
            >
              <a-date-picker
                v-model:value="leaveForm.startDate"
          ...
          <a-col :span="8">
            <a-form-item label="结束日期" name="endDate" :rules="requiredRule">
              <a-date-picker
                v-model:value="leaveForm.endDate"
                value-format="YYYY-MM-DD"
                style="width: 100%"
                @change="calcDays"
              />
          ...
          <a-col :span="8">
            <a-form-item label="共计天数">
              <a-input-number
                v-model:value="leaveForm.totalDays"
                :min="0.5"
                :step="0.5"
              />
            </a-form-item>
...
const pendingCount = computed(
  () => data.value.filter((d) => d.status === "PENDING").length,
);
const monthlyCount = computed(() => {
  ...
  return data.value.filter((d) => {
    ...
    return (
      s.getMonth() === now.getMonth() && s.getFullYear() === now.getFullYear()
    );
  }).length;
});
...
function leaveApprovalSteps(item: LeaveRecord): ApprovalProgressStep[] {
  return [
    {
      key: "start",
      personName: item.employeeName || "申请人",
      title: "发起申请",
      time: item.startDate,
