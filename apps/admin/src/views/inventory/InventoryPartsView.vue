  ...
  <div class="page-stack">
    <a-card title="库存管理">
      ...
      <a-row :gutter="[16, 16]" class="metric-row">
        <a-col :xs="12" :lg="6"
          ><a-statistic title="物料品种" :value="parts.length" suffix="种"
        /></a-col>
        <a-col :xs="12" :lg="6"
          ><a-statistic
            title="库存金额"
        ...
        /></a-col>
        <a-col :xs="12" :lg="6"
          ><a-statistic title="低库存" :value="lowStockCount" suffix="种"
        /></a-col>
        <a-col :xs="12" :lg="6"
          ><a-statistic
            title="项目材料净耗用"
            :value="netIssueAmount"
            :formatter="moneyFormatter"
        /></a-col>
      </a-row>
      ...
      <a-tabs v-model:active-key="activeTab">
            ...
            <template #bodyCell="{ column, record }">
              <template v-if="column.key === 'name'">
                ...
                <span class="table-subtitle"
                  >{{ record.code }} · {{ record.model || "未填型号" }}</span
                >
              </template>
              <template v-else-if="column.key === 'stock'">
                <strong>{{ formatQuantity(record.stockQty) }}</strong>
                <span class="table-subtitle"
                  >安全库存 {{ formatQuantity(record.safetyQty) }}</span
                >
              </template>
              ...
              <template v-else-if="column.key === 'value'"
                ><strong>{{
                  formatMoney(record.stockQty * record.unitCost)
                }}</strong></template
              >
              <template v-else-if="column.key === 'action'">
                <a-space size="small">
                  <a-button
                    v-if="auth.can('inventory:movement:create')"
                    type="link"
                    size="small"
                    @click="openMovement(record)"
                    >库存调整</a-button
                  >
                  <a-button
                    type="link"
                    size="small"
                    @click="openMovements(record)"
                    >流水</a-button
                  >
                </a-space>
              </template>
        ...
        <a-tab-pane key="issues" tab="项目领料">
          <div class="table-toolbar">
            <a-button
              v-if="auth.can('inventory:issue:create')"
              type="primary"
              @click="openIssue"
            >
          ...
          <a-table
            ...
            :loading="loading"
            :pagination="{ pageSize: 8 }"
            :row-key="(record: MaterialIssue) => record.id"
            :scroll="{ x: 1250 }"
            size="middle"
          >
            <template #bodyCell="{ column, record }">
              <template v-if="column.key === 'code'"
                ...
                ><span class="table-subtitle"
                  >{{ record.issueDate }} · {{ record.receiverName }}</span
                ></template
              >
              ...
              <template v-else-if="column.key === 'lines'"
                >{{ issueLineSummary(record)
                }}<span class="table-subtitle"
                  >共 {{ record.lines.length }} 项</span
                ></template
              >
              ...
              <template v-else-if="column.key === 'action'">
                <a-button
                  v-if="
                    auth.can('inventory:return:create') &&
                    returnableLines(record).length
                  "
                  type="link"
                  size="small"
        ...
        <a-tab-pane key="returns" tab="项目退料">
          <a-table
            ...
            :loading="loading"
            :pagination="{ pageSize: 8 }"
            :row-key="(record: MaterialReturn) => record.id"
            :scroll="{ x: 1050 }"
            size="middle"
          >
            <template #bodyCell="{ column, record }">
    ...
    <a-drawer
      v-model:open="movementDrawerOpen"
      :width="760"
      :title="selectedPart ? `${selectedPart.name} 库存流水` : '库存流水'"
    >
      <template #extra
        ><a-tag
          v-if="selectedPart"
          :color="selectedPart.lowStock ? 'red' : 'green'"
          >当前库存 {{ formatQuantity(selectedPart.stockQty) }}</a-tag
        ></template
      >
      <a-table
    ...
    <a-modal
      ...
      :title="movementPart ? `${movementPart.name} 库存调整` : '库存调整'"
      width="640px"
      :confirm-loading="saving"
    ...
    >
      ...
      >
        <a-row :gutter="16">
          <a-col :xs="24" :md="12"
            ><a-form-item label="调整类型" name="movementType"
              ><a-select
                v-model:value="movementForm.movementType"
                :options="manualMovementOptions" /></a-form-item
          ></a-col>
          <a-col :xs="24" :md="12"
          ...
          <a-col :xs="24" :md="12"
            ><a-form-item label="来源单号"
              ><a-input v-model:value="movementForm.sourceNo" /></a-form-item
          ></a-col>
          <a-col :xs="24" :md="12"
            ><a-form-item label="备注"
              ><a-input v-model:value="movementForm.remark" /></a-form-item
          ></a-col>
        </a-row>
      </a-form>
    </a-modal>
    ...
    >
      ...
      >
        <a-row :gutter="16">
          ...
          ></a-col>
          <a-col :xs="24" :md="6"
            ><a-form-item label="领料日期" name="issueDate"
              ><a-input
                v-model:value="issueForm.issueDate"
                type="date" /></a-form-item
          ></a-col>
          <a-col :xs="24" :md="8"
            ><a-form-item label="领用人" name="receiverName"
              ><a-input v-model:value="issueForm.receiverName" /></a-form-item
          ></a-col>
          <a-col :xs="24" :md="16"
            ><a-form-item label="领料用途" name="purpose"
        ...
        >
          <a-row :gutter="12" align="middle">
            ...
            /></a-col>
            <a-col :span="5"
              ><span>{{ formatMoney(lineAmount(line)) }}</span></a-col
            >
            <a-col :span="3"
              ><a-button
                danger
                ...
                @click="removeIssueLine(index)"
                >移除</a-button
              ></a-col
            >
          </a-row>
        </div>
        <a-button type="dashed" block @click="addIssueLine"
          ><template #icon><PlusOutlined /></template>增加物料明细</a-button
        >
      </a-form>
    </a-modal>
    ...
    <a-modal
      ...
      width="820px"
      :confirm-loading="saving"
      @ok="handleCreateReturn"
    >
      <a-alert
        v-if="activeIssue"
        class="section-alert"
        type="info"
        :message="`${activeIssue.code} · ${activeIssue.projectName} · 可退 ${returnableLines(activeIssue).length} 项`"
      />
      <a-form
        ref="returnFormRef"
        :model="returnForm"
      ...
      >
        <a-row :gutter="16">
          <a-col :xs="24" :md="8"
            ><a-form-item label="退料日期" name="returnDate"
              ><a-input
                v-model:value="returnForm.returnDate"
                type="date" /></a-form-item
          ></a-col>
          <a-col :xs="24" :md="8"
            ><a-form-item label="经办人" name="handlerName"
        ...
        <a-table
          ...
          :pagination="false"
          :row-key="(line: ReturnFormLine) => line.issueLineId"
          size="small"
        >
          <template #bodyCell="{ column, record }">
            <template v-if="column.key === 'part'"
              >{{ record.partName
                ...
                >单价 {{ formatMoney(record.unitCost) }}</span
              ></template
            >
            <template v-else-if="column.key === 'returnable'">{{
              formatQuantity(record.returnableQty)
            }}</template>
...
const issueRules = {
  ...
  receiverName: [{ required: true, message: "请输入领用人" }],
  purpose: [{ required: true, message: "请输入领料用途" }],
};
const returnRules = {
  code: [],
...
const inventoryValue = computed(() =>
  parts.value.reduce(
    (sum, item) => sum + Number(item.stockQty) * Number(item.unitCost),
    0,
  ),
);
const lowStockCount = computed(
  () => parts.value.filter((item) => item.lowStock).length,
);
const netIssueAmount = computed(
  () =>
    issues.value.reduce((sum, item) => sum + Number(item.totalAmount), 0) -
    returns.value.reduce((sum, item) => sum + Number(item.totalAmount), 0),
);
const partOptions = computed(() =>
  parts.value.map((item) => ({
    label: `${item.name} (${item.code}) · 库存 ${formatQuantity(item.stockQty)}`,
    value: item.id,
  })),
);
const projectOptions = computed(() =>
  eligibleProjects.value.map((item) => ({
    label: `${item.code} · ${item.name} · ${item.managerName}`,
    value: item.id,
  })),
);
const issueTotal = computed(() =>
  issueForm.lines.reduce((sum, item) => sum + lineAmount(item), 0),
);
const returnTotal = computed(() =>
  returnForm.lines.reduce(
    (sum, item) =>
...
async function loadData() {
  ...
  errorMessage.value = "";
  try {
    [parts.value, issues.value, returns.value, eligibleProjects.value] =
      await Promise.all([
        listInventoryParts(),
        listMaterialIssues(),
        listMaterialReturns(),
    ...
    if (selectedPart.value)
      selectedPart.value =
        parts.value.find((item) => item.id === selectedPart.value?.id) || null;
  } catch (error) {
    errorMessage.value =
      error instanceof Error ? error.message : "仓储数据加载失败";
  } finally {
    loading.value = false;
  }
}
...
function openIssue() {
  if (!eligibleProjects.value.length) {
    message.warning("暂无审批通过且未关闭的项目");
    return;
  }
  Object.assign(issueForm, initialIssueForm());
  issueOpen.value = true;
}
function openReturn(issue: MaterialIssue) {
  activeIssue.value = issue;
  Object.assign(returnForm, initialReturnForm(), {
    lines: returnableLines(issue).map((line) => ({
      issueLineId: line.id,
      partName: line.partName,
      returnableQty: Number(line.returnableQty),
...
function addIssueLine() {
  issueForm.lines.push({ key: ++lineKey, partId: "", quantity: 1 });
}
...
async function handleCreateMovement() {
  ...
  try {
    ...
    message.success("库存调整已完成");
    await loadData();
  } catch (error) {
    message.error(error instanceof Error ? error.message : "库存调整失败");
  } finally {
    saving.value = false;
  }
}
async function handleCreateIssue() {
  ...
  if (
    new Set(issueForm.lines.map((line) => line.partId)).size !==
    issueForm.lines.length
  ) {
    message.warning("同一物料不能重复添加");
    return;
  }
  saving.value = true;
  try {
    await createMaterialIssue({
      ...
      lines: issueForm.lines.map((line) => ({
        partId: line.partId,
    ...
    message.success("领料完成，库存和项目材料成本已同步");
    await loadData();
  } catch (error) {
    message.error(error instanceof Error ? error.message : "项目领料失败");
  } finally {
    saving.value = false;
  }
}
async function handleCreateReturn() {
  ...
  saving.value = true;
  try {
    await createMaterialReturn(activeIssue.value.id, {
    ...
    message.success("退料完成，库存已恢复并冲回项目材料成本");
    await loadData();
  } catch (error) {
    message.error(error instanceof Error ? error.message : "项目退料失败");
  } finally {
    saving.value = false;
  }
}
...
function initialIssueForm(): IssueForm {
  return {
    code: "",
    projectId: "",
    issueDate: today(),
    ...
    purpose: "",
    lines: [{ key: ++lineKey, partId: "", quantity: 1 }],
  };
}
function initialReturnForm(): ReturnForm {
  return {
    code: "",
    returnDate: today(),
...
function lineAmount(line: IssueFormLine) {
  const part = parts.value.find((item) => item.id === line.partId);
  return Number(line.quantity || 0) * Number(part?.unitCost || 0);
}
function returnableLines(issue: MaterialIssue) {
  return issue.lines.filter((line) => Number(line.returnableQty) > 0);
}
function issueLineSummary(issue: MaterialIssue) {
  return issue.lines
    .map((line) => `${line.partName} × ${formatQuantity(line.quantity)}`)
    .join("，");
}
function returnLineSummary(item: MaterialReturn) {
  return item.lines
    .map((line) => `${line.partName} × ${formatQuantity(line.quantity)}`)
    .join("，");
}
function issueStatusLabel(status: InventoryIssueStatus) {
  return (
    {
      POSTED: "已领料",
      PARTIAL_RETURNED: "部分退料",
      FULLY_RETURNED: "全部退料",
    } as Record<InventoryIssueStatus, string>
  )[status];
}
function issueStatusColor(status: InventoryIssueStatus) {
  return (
    {
      POSTED: "blue",
