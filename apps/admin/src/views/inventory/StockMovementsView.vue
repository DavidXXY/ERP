        ...
        <template #bodyCell="{ column, record }">
          <template v-if="column.key === 'type'"
            ><a-tag
              ...
              "
              >{{
                statusLabel(record.movementType, {
                  INBOUND: "入库",
                  OUTBOUND: "出库",
                  RETURN: "退回",
                  SCRAP: "报废",
...
import { listAllStockMovements, type StockMovement } from "@/api/inventory";
import { statusLabel, statusColor } from "@/utils/status-mapper";
const router = useRouter();
const loading = ref(false);
const movements = ref<StockMovement[]>([]);
const movementColumns = [
  { title: "物料", dataIndex: "partName", width: 160 },
...
async function loadData() {
  loading.value = true;
  try {
    movements.value = await listAllStockMovements();
  } catch (e: any) {
