    ...
    <a-card>
      ...
      <a-table :columns="movementColumns" :data-source="movements" :loading="loading" :pagination="{pageSize:10}" :row-key="(r:any)=>r.id" size="small">
        <template #bodyCell="{ column, record }">
          <template v-if="column.key === 'type'"><a-tag :color="statusColor(record.movementType, {INBOUND:'green',OUTBOUND:'red',RETURN:'blue',SCRAP:'orange',ADJUSTMENT:'purple'}, 'default')">{{ statusLabel(record.movementType, {INBOUND:'入库',OUTBOUND:'出库',RETURN:'退回',SCRAP:'报废',ADJUSTMENT:'调整'}) }}</a-tag></template>
...
import { listInventoryParts } from "@/api/inventory";
...
onMounted(loadData);
async function loadData(){loading.value=true;try{const parts=await listInventoryParts();const partId=parts[0]?.id||'';movements.value=partId?await listStockMovements(partId):[];}catch(e:any){message.error(e.message||'加载失败');}finally{loading.value=false;}}
</script>
