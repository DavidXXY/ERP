  ...
  <div class="page-stack">
    <a-card title="操作审计日志">
      ...
      >
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.key === 'module'"><a-tag>{{ record.bizModule || auditObjectLabel(record.requestPath) }}</a-tag></template>
          ...
          <template v-else-if="column.key === 'action'"><a-button type="link" size="small" @click="openDetail(record)">详情</a-button></template>
        </template>
      </a-table>
    </a-card>
    ...
    <a-drawer v-model:open="detailOpen" title="审计详情" :width="560">
      <a-descriptions v-if="selected" bordered :column="1" size="small">
        <a-descriptions-item label="操作人">{{ selected.username || '-' }}</a-descriptions-item>
        <a-descriptions-item label="请求方法"><a-tag :color="methodColor(selected.httpMethod)">{{ selected.httpMethod }}</a-tag></a-descriptions-item>
        <a-descriptions-item label="业务对象">{{ auditObjectLabel(selected.requestPath) }}</a-descriptions-item>
        <a-descriptions-item label="业务模块">{{ selected.bizModule || '-' }}</a-descriptions-item>
        <a-descriptions-item label="对象ID">{{ selected.bizObject || '-' }}</a-descriptions-item>
        <a-descriptions-item label="操作类型">{{ selected.operationType || '-' }}</a-descriptions-item>
        <a-descriptions-item label="请求路径">{{ selected.requestPath }}</a-descriptions-item>
        <a-descriptions-item label="查询参数">{{ selected.queryString || '-' }}</a-descriptions-item>
        <a-descriptions-item label="响应状态"><a-tag :color="selected.responseStatus < 300 ? 'green' : selected.responseStatus < 400 ? 'orange' : 'red'">{{ selected.responseStatus }}</a-tag></a-descriptions-item>
        <a-descriptions-item label="客户端 IP">{{ selected.clientIp || '-' }}</a-descriptions-item>
        <a-descriptions-item label="耗时">{{ selected.durationMs }}ms</a-descriptions-item>
        <a-descriptions-item label="操作时间">{{ selected.createdAt?.slice(0, 19)?.replace('T', ' ') || '-' }}</a-descriptions-item>
      </a-descriptions>
    </a-drawer>
  </div>
</template>
...
function exportAudits() {
  downloadCsv("操作审计日志.csv", ["操作人", "方法", "模块", "操作类型", "业务对象", "路径", "查询参数", "状态", "IP", "耗时(ms)", "时间"], auditLogs.value.map((item) => [
    item.username || "",
    item.httpMethod,
    item.bizModule || "",
    item.operationType || "",
