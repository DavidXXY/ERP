  ...
  <div class="page-stack">
    <a-card>
      ...
      <a-space wrap class="table-toolbar">
        <a-button v-if="auth.can('office:expense:create')" type="primary" @click="openExpense">
          <template #icon><PlusOutlined /></template>新增报销
        </a-button>
      </a-space>

      <a-table :columns="expenseColumns" :data-source="expenses" :loading="loading" :pagination="{ pageSize: 10 }" :row-key="(r: Expense) => r.id" :scroll="{ x: 1260 }">
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.key === 'lines'">{{ record.lines?.length || 1 }} 行</template>
    ...
    <a-modal v-model:open="expenseOpen" title="新增费用报销" width="980px" :confirm-loading="saving" @ok="handleExpense">
      <a-form ref="expenseFormRef" :model="expenseForm" :rules="expenseRules" layout="vertical">
        <a-row :gutter="16">
          <a-col :xs="24" :md="12">
            <a-form-item label="报销人" name="claimantId">
              <a-select v-model:value="expenseForm.claimantId" show-search option-filter-prop="label" :options="userOptions" @change="onClaimantChange" />
            </a-form-item>
          </a-col>
          <a-col :xs="24" :md="12">
            <a-form-item label="绑定项目">
              <a-select v-model:value="expenseForm.projectId" allow-clear show-search option-filter-prop="label" :options="projectOptions" />
            </a-form-item>
          </a-col>
          <a-col :xs="24" :md="12">
            <a-form-item label="绑定工单">
              <a-select v-model:value="expenseForm.workOrderId" allow-clear show-search option-filter-prop="label" :options="workOrderOptions" />
            </a-form-item>
          </a-col>
          <a-col :xs="24" :md="12">
            <a-form-item label="报销总额">
              <a-input-number :value="expenseTotal" disabled :precision="2" class="full-input" />
            </a-form-item>
          </a-col>
        </a-row>

        <a-form-item label="报销明细">
          <a-table :columns="lineColumns" :data-source="expenseForm.lines" :pagination="false" :row-key="(row: ExpenseDraftLine) => row.rowKey" size="small" :scroll="{ x: 980 }">
            <template #bodyCell="{ column, record, index }">
              <template v-if="column.key === 'expenseType'">
                <a-select v-model:value="record.expenseType" :options="expenseTypeOptions" style="width: 120px" />
              </template>
              <template v-else-if="column.key === 'expenseDate'">
                <a-input v-model:value="record.expenseDate" type="date" style="width: 138px" />
              </template>
              <template v-else-if="column.key === 'amount'">
                <a-input-number v-model:value="record.amount" :min="0" :precision="2" style="width: 120px" />
              </template>
              <template v-else-if="column.key === 'description'">
                <a-input v-model:value="record.description" placeholder="费用说明" />
              </template>
              <template v-else-if="column.key === 'invoice'">
                <a-upload :before-upload="(file: File) => bindInvoiceFile(record, file)" :show-upload-list="false" accept=".jpg,.jpeg,.png,.pdf,.ofd">
                  <a-button size="small">上传发票</a-button>
                </a-upload>
                <span v-if="record.invoiceFileName" class="table-subtitle">{{ record.invoiceFileName }}</span>
              </template>
              <template v-else-if="column.key === 'action'">
                <a-button type="link" danger size="small" :disabled="expenseForm.lines.length === 1" @click="removeLine(index)">删除</a-button>
              </template>
            </template>
          </a-table>
          <a-button type="dashed" block style="margin-top: 8px" @click="addLine">新增一行</a-button>
        </a-form-item>
      </a-form>
    </a-modal>
  </div>
...
import { createExpense, getOfficeReferences, listExpenses, uploadDocument, type Expense, type ExpenseLine, type ExpenseStatus, type ExpenseType } from "@/api/office";
...
const expenseRules = { claimantId: [{ required: true, message: "请选择报销人" }] };
...
const expenseTotal = computed(() => expenseForm.lines.reduce((sum, line) => sum + Number(line.amount || 0), 0));
...
async function loadData() {
  loading.value = true;
  try {
    const [expenseData, referenceData] = await Promise.all([listExpenses(), getOfficeReferences()]);
...
function onClaimantChange(id: string) {
  expenseForm.claimantName = references.users.find((i: any) => i.id === id)?.displayName || "";
}
function newLine(): ExpenseDraftLine {
  return { rowKey: `${Date.now()}-${Math.random()}`, expenseType: "TRAVEL", amount: 0, expenseDate: today(), description: "" };
}
function addLine() { expenseForm.lines.push(newLine()); }
function removeLine(index: number) { expenseForm.lines.splice(index, 1); }
...
async function handleExpense() {
  ...
  const draftLines = expenseForm.lines.filter((line) => line.amount > 0 && line.expenseDate && line.description.trim());
  const lines = draftLines.map(({ rowKey, invoiceFile, ...line }) => line);
  if (!lines.length) { message.warning("请至少填写一条完整报销明细"); return; }
  saving.value = true;
  try {
...
async function uploadInvoiceFiles(created: Expense, draftLines: ExpenseDraftLine[]) {
  const tasks = draftLines
    .map((line, index) => ({ file: line.invoiceFile, responseLine: created.lines?.[index] }))
    .filter((item): item is { file: File; responseLine: ExpenseLine } => Boolean(item.file && item.responseLine?.id));
  if (!tasks.length) return;
  const results = await Promise.allSettled(tasks.map((item) => uploadDocument({ bizType: "EXPENSE_INVOICE", bizId: item.responseLine.id, file: item.file })));
  const failed = results.filter((item) => item.status === "rejected").length;
  if (failed) message.warning(`${failed} 个发票文件上传失败，报销明细已保存，可稍后补传。`);
}
