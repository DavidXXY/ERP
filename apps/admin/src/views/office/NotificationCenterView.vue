<template>
  <div class="page-stack">
    <a-card>
      ...
      <a-list bordered :data-source="filteredNotifications" :loading="loading" class="notification-list">
        <template #renderItem="{ item }">
          <a-list-item :class="{'notification-unread':!item.read}" class="notification-item">
            <div class="notification-body">
              <div class="notification-head">
                <div class="notification-title-row">
                  <span :class="{'notification-title-unread':!item.read}" class="notification-title">{{ item.title }}</span>
              ...
              <div class="notification-content">{{ item.content || "无消息内容" }}</div>
            ...
            <template #actions>
              ...
              <a-button v-if="relatedRoute(item)" type="link" size="small" @click.stop="goRelated(item)">查看关联</a-button>
            </template>
          </a-list-item>
        </template>
...
const filteredNotifications=computed(()=>notifications.value.filter(item=>readFilter.value==='all'||(readFilter.value==='unread'?!item.read:item.read)));
onMounted(loadData);
...
function relatedRoute(item:NotificationRecord){
  ...
  if(type.includes('PART')||type.includes('INVENTORY'))return '/inventory/parts';
  return '';
}
async function goRelated(item:NotificationRecord){if(!item.read)await handleRead(item);const route=relatedRoute(item);if(route)router.push(route);}
function typeLabel(type?:string){return ({APPROVAL:'审批',APPROVAL_RESULT:'审批结果',APPROVAL_SLA:'审批SLA',APPROVAL_ESCALATED:'审批升级',EQUIPMENT:'设备',CERTIFICATE:'证书',CONTRACT:'合同',FINANCE:'财务',INVENTORY:'库存',OUTSOURCE:'外包'} as Record<string,string>)[type||'']||type||'消息';}
function typeColor(type?:string){if(!type)return 'default';if(type.includes('APPROVAL'))return 'blue';if(type.includes('SLA')||type.includes('ESCALATED'))return 'red';if(type==='FINANCE')return 'green';if(type==='INVENTORY')return 'orange';return 'default';}

function formatDateTime(v?:string){return v?new Date(v).toLocaleString('zh-CN',{hour12:false}):'-';}
</script>
<style scoped>
.notification-list :deep(.ant-list-item-action){margin-left:16px}
.notification-item{align-items:flex-start}
.notification-unread{background:#f0f7ff}
.notification-title-unread{font-weight:600}
.notification-body{width:100%;min-width:0}
.notification-head{display:flex;align-items:flex-start;justify-content:space-between;gap:16px;margin-bottom:8px}
.notification-title-row{display:flex;min-width:0;flex-wrap:wrap;align-items:center;gap:6px}
.notification-title{font-size:15px;color:#101828;word-break:break-word}
.notification-time{flex:none;color:#8c8c8c;font-size:12px}
.notification-content{white-space:pre-wrap;word-break:break-word;line-height:1.7;color:#344054}
.notification-meta{display:flex;flex-wrap:wrap;gap:12px;margin-top:10px;color:#8c8c8c;font-size:12px}
@media (max-width: 720px){.notification-head{display:block}.notification-time{display:block;margin-top:6px}}
</style>
