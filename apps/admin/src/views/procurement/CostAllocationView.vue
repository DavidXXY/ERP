<template>
  <div class="page-stack">
    <a-card>
      ...
      <a-table :columns="costColumns" :data-source="costs" :loading="loading" :pagination="{pageSize:10}" :row-key="(r:any)=>r.id" :scroll="{x:1100}">
        <template #bodyCell="{ column, record }">
          <template v-if="column.key === 'cost'"><strong>{{ record.code || '-' }}</strong><span class="table-subtitle">{{ record.description || '' }}</span></template>
          <template v-else-if="column.key === 'type'">{{ {PROJECT:'项目',DEPARTMENT:'部门',OTHER:'其他'}[record.costType]||record.costType }}</template>
          <template v-else-if="column.key === 'amount'">{{ formatMoney(record.amount) }}</template>
          <template v-else-if="column.key === 'date'">{{ record.createdAt?.slice(0,10) || '-' }}</template>
        </template>
      </a-table>
    </a-card>
  </div>
...
onMounted(loadData);
async function loadData(){loading.value=true;try{costs.value=await listProcurementCostAllocations();}catch(e:any){message.error(e.message||'加载失败');}finally{loading.value=false;}}
