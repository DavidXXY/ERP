      ...
      >
        <template #bodyCell="{ column, record }">
          <template v-if="column.key === 'receipt'"
            ><strong>{{ record.receiptCode || record.id?.slice(0, 8) }}</strong
          ...
          <template v-else-if="column.key === 'amount'"
            >{{ formatMoney(record.amount) }}<br /><span class="table-subtitle"
          ...
          <template v-else-if="column.key === 'status'"
            ><a-tag color="green">已入库</a-tag></template
          >
        </template>
      </a-table>
    </a-card>
...
async function loadData() {
  loading.value = true;
  try {
    receipts.value = await listGoodsReceipts();
  } catch (e: any) {
