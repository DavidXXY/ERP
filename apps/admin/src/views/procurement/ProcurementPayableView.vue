<template>
  <div class="page-stack">
    <a-card>
      ...
      <a-table :columns="payableColumns" :data-source="payables" :loading="loading" :pagination="{pageSize:10}" :row-key="(r:any)=>r.id" :scroll="{x:1100}">
        <template #bodyCell="{ column, record }">
          <template v-if="column.key === 'payable'"><strong>{{ record.code }}</strong><span class="table-subtitle">{{ record.supplierName || '' }}</span></template>
...
onMounted(loadData);
async function loadData(){loading.value=true;try{payables.value=await listProcurementPayables();}catch(e:any){message.error(e.message||'加载失败');}finally{loading.value=false;}}
