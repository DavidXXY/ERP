  ...
  <div class="page-stack">
    <a-card title="供应链采购">
      ...
      <a-row :gutter="[16, 16]" class="metric-row">
        ...
        /></a-col>
        <a-col :xs="12" :lg="8" :xl="4"
          ><a-statistic title="待收货订单" :value="openOrderCount" suffix="单"
        /></a-col>
        <a-col :xs="12" :lg="8" :xl="4"
          ><a-statistic
            title="累计入库金额"
        ...
        <a-tab-pane key="requests" tab="采购申请">
          <div class="table-toolbar">
            ...
            <a-space class="filter-space">
              ...
              />
              <a-select
                v-model:value="requestFilters.approvalStatus"
                allow-clear
                placeholder="审批状态"
                style="width: 120px"
              ...
              />
              <a-select
                v-model:value="requestFilters.costType"
                allow-clear
                placeholder="成本归属"
                style="width: 120px"
              ...
              />
              <a-input-search
                v-model:value="requestFilters.search"
                placeholder="搜索申请编号/物料/申请人"
                style="width: 260px"
                @search="handleRequestFilter"
              />
          ...
          <a-table
            :row-selection="{
              ...
              onChange: (keys: any) => (selectedRequestKeys.values = keys),
            }"
            ...
            <template #bodyCell="{ column, record }">
              <template v-if="column.key === 'code'">
                ...
                <span class="table-subtitle"
                  ...
                  {{ record.reason || "无备注" }}</span
                >
              </template>
              <template v-else-if="column.key === 'part'">
                {{ record.partName }}
                <span class="table-subtitle"
                  >申请数量 {{ formatQuantity(record.quantity) }}</span
                >
                <span class="table-subtitle"
                  >预计 {{ formatMoney(record.totalAmount || 0) }} · 税率
              ...
              <template v-else-if="column.key === 'status'">
                <a-tag :color="requestColor(record.status)">{{
                  requestLabel(record.status)
                }}</a-tag>
                ...
                <span v-if="record.lastApproverName" class="table-subtitle">
                  {{ record.lastApproverName }} ·
                  {{ record.lastApprovalComment }}
                </span>
              </template>
        ...
        <a-tab-pane key="orders" tab="采购订单">
          <div class="table-toolbar">
            <a-space wrap>
              ...
              <span class="muted"
                >当前有 {{ requestOptions.length }} 条已审批申请可下单</span
              >
              <a-button
                style="float: right"
                size="small"
                @click="handleRequestFilter"
                >刷新列表</a-button
              >
            </a-space>
            <a-space class="filter-space">
              ...
              />
              <a-select
                v-model:value="orderFilters.costType"
                allow-clear
                placeholder="成本归属"
                style="width: 120px"
                :options="costTypeOptions"
              />
              <a-input-search
                v-model:value="orderFilters.search"
                placeholder="搜索订单编号/物料"
                style="width: 220px"
                @search="handleRequestFilter"
              />
              <a-button @click="handleRequestFilter">查询</a-button>
          ...
          >
            <template #bodyCell="{ column, record }">
              ...
              <template v-else-if="column.key === 'quantity'">
                <strong
                  >{{ formatQuantity(record.receivedQty) }} /
                  {{ formatQuantity(record.orderedQty) }}</strong
                >
                <a-progress
              ...
              <template v-else-if="column.key === 'status'">
                <a-tag :color="orderColor(record.status)">{{
                  orderLabel(record.status)
                }}</a-tag>
              </template>
              <template v-else-if="column.key === 'action'">
                <a-space>
          ...
          >
            <template #bodyCell="{ column, record }">
              ...
              <template v-else-if="column.key === 'amount'">
                <strong>{{ formatMoney(record.amount) }}</strong>
                <span class="table-subtitle"
                  >待付 {{ formatMoney(record.outstandingAmount) }} · 税率
                  {{ formatTaxRate(record.taxRate) }}</span
                >
              </template>
              <template v-else-if="column.key === 'costTarget'">
              ...
              <template v-else-if="column.key === 'status'">
                <a-tag :color="payableColor(record.status)">{{
                  payableLabel(record.status)
                }}</a-tag>
          ...
          >
            <template #bodyCell="{ column, record }">
              <template v-if="column.key === 'name'">
                ...
                <span class="table-subtitle"
                  >{{ record.code }} · {{ record.category || "未分类" }}</span
                >
              </template>
              <template v-else-if="column.key === 'risk'">
                <a-tag :color="supplierRiskColor(record.riskStatus)">{{
                  supplierRiskLabel(record.riskStatus)
                }}</a-tag>
              </template>
            </template>
          </a-table>
        ...
        <a-row :gutter="16">
          ...
          <a-col :xs="24" :md="12"
            ><a-form-item label="账期"
              ><a-input
                v-model:value="supplierForm.settlementTerms"
                placeholder="例如：月结30天" /></a-form-item
          ></a-col>
          <a-col :xs="24" :md="8"
            ><a-form-item label="联系人"
              ><a-input v-model:value="supplierForm.contactName" /></a-form-item
          ></a-col>
          <a-col :xs="24" :md="8"
            ><a-form-item label="电话"
              ><a-input v-model:value="supplierForm.phone" /></a-form-item
          ></a-col>
          <a-col :xs="24" :md="8"
            ><a-form-item label="风险状态"
              ><a-select
                v-model:value="supplierForm.riskStatus"
    ...
    <a-modal
      ...
      :title="editingRequestId ? '编辑采购申请' : '新增采购申请'"
      width="760px"
      :confirm-loading="savingRequest"
      @ok="handleSaveRequest"
    >
      <a-form
        ref="requestFormRef"
        :model="requestForm"
        ...
        <a-row :gutter="16">
          <a-col :xs="24" :md="8"
            ><a-form-item label="申请人" name="requesterName"
              ><a-input
                v-model:value="requestForm.requesterName" /></a-form-item
          ></a-col>
          <a-col :xs="24" :md="8"
            ><a-form-item label="期望到货"
              ><a-input
                v-model:value="requestForm.expectedDate"
                type="date" /></a-form-item
          ></a-col>
          <a-col :xs="24" :md="12"
            ><a-form-item label="关联物料" name="partId"
              ><a-select
                ...
                placeholder="选择物料"
                @change="syncRequestPart" /></a-form-item
          ></a-col>
          <a-col :xs="24" :md="12"
          ...
          ></a-col>
          <a-col :span="24"
            ><a-form-item label="成本归属" name="costType"
              ><a-segmented
                v-model:value="requestForm.costType"
                :options="costTypeOptions"
                @change="changeCostType" /></a-form-item
          ></a-col>
          <a-col v-if="requestForm.costType === 'PROJECT'" :span="24"
            ><a-form-item
              label="关联项目"
              name="projectId"
              :rules="[{ required: true, message: '请选择成本项目' }]"
              ><a-select
                v-model:value="requestForm.projectId"
          ...
          <a-col v-else :span="24"
            ><a-form-item
              label="成本部门"
              name="departmentId"
              :rules="[{ required: true, message: '请选择成本部门' }]"
              ><a-select
                v-model:value="requestForm.departmentId"
          ...
          <a-col :xs="24" :md="8"
            ><a-form-item label="税率(%)"
              ><a-input-number
                v-model:value="requestForm.taxRate"
                :min="0"
                :max="100"
                :precision="2"
                class="full-input" /></a-form-item
          ...
          <a-col :xs="24" :md="16"
            ><a-form-item label="采购原因"
              ><a-input v-model:value="requestForm.reason" /></a-form-item
          ></a-col>
        </a-row>
      </a-form>
    </a-modal>
    ...
    <a-modal
      ...
      title="采购申请审批"
      width="720px"
      :confirm-loading="savingApproval"
      @ok="handleApproval"
    >
      <a-alert
        ...
        class="section-alert"
        type="info"
        :message="`${selectedRequest.code} · ${selectedRequest.partName} · ${costTypeLabel(selectedRequest.costType)}：${selectedRequest.costTargetName}`"
      />
      <a-form
        ref="approvalFormRef"
        :model="approvalForm"
        ...
        <a-row :gutter="16">
          ...
          <a-col :xs="24" :md="14"
            ><a-form-item label="审批人" name="approverName"
              ><a-input
                v-model:value="approvalForm.approverName" /></a-form-item
          ></a-col>
          <a-col :span="24"
    ...
    >
      ...
      >
        <a-row :gutter="16">
          <a-col :xs="24" :md="16"
            ><a-form-item label="关联采购申请" name="requestId"
              ><a-select
                v-model:value="orderForm.requestId"
                :options="requestOptions"
                show-search
                option-filter-prop="label"
                placeholder="仅显示已审批且未下单的申请"
                @change="syncOrderRequest" /></a-form-item
          ></a-col>
          <a-col :xs="24" :md="16"
          ...
          <a-col :xs="24" :md="8"
            ><a-form-item label="预计到货"
              ><a-input
                v-model:value="orderForm.expectedDeliveryDate"
                type="date" /></a-form-item
          ></a-col>
          <a-col :xs="24" :md="8"
          ...
          <a-col :xs="24" :md="8"
            ><a-form-item label="税率(%)"
              ><a-input-number
                v-model:value="orderForm.taxRate"
                :min="0"
                :max="100"
                :precision="2"
                class="full-input" /></a-form-item
              ...
              ><a-input
                :value="
                  selectedOrderRequest
                    ? `${costTypeLabel(selectedOrderRequest.costType)} · ${selectedOrderRequest.costTargetName}`
                    : ''
                "
                disabled /></a-form-item
          ></a-col>
    ...
    >
      <a-alert
        ...
        class="section-alert"
        type="info"
        :message="`${selectedOrder.code} · 剩余可收 ${formatQuantity(remainingQuantity)} · ${costTypeLabel(selectedOrder.costType)}：${selectedOrder.costTargetName}`"
      />
      <a-form
        ref="receiptFormRef"
        :model="receiptForm"
      ...
      >
        <a-row :gutter="16">
          <a-col :xs="24" :md="8"
            ><a-form-item label="本次到货数量" name="quantity"
              ><a-input-number
                v-model:value="receiptForm.quantity"
                :min="0.01"
          ...
          ></a-col>
          <a-col :xs="24" :md="8"
            ><a-form-item label="到货日期" name="receivedDate"
              ><a-input
                v-model:value="receiptForm.receivedDate"
                type="date" /></a-form-item
          ></a-col>
          <a-col :xs="24" :md="8"
            ><a-form-item label="应付到期日" name="payableDueDate"
              ><a-input
                v-model:value="receiptForm.payableDueDate"
                type="date" /></a-form-item
          ></a-col>
          <a-col :xs="24" :md="12"
            ><a-form-item label="送货单号" name="deliveryNo"
...
const costFilterOptions = [{ label: "全部", value: "ALL" }, ...costTypeOptions];
const supplierRules = {
...
const pendingApprovalCount = computed(
  () =>
    purchaseRequests.value.filter((item) => item.approvalStatus === "PENDING")
      .length,
);
const openOrderCount = computed(
  () => purchaseOrders.value.filter(canReceive).length,
);
const receiptAmount = computed(() =>
  goodsReceipts.value.reduce((sum, item) => sum + Number(item.amount || 0), 0),
);
const outstandingPayable = computed(() =>
  payables.value.reduce(
    (sum, item) => sum + Number(item.outstandingAmount || 0),
    0,
  ),
);
const projectCostAmount = computed(() =>
  costAllocations.value
    .filter((item) => item.costType === "PROJECT")
    .reduce((sum, item) => sum + Number(item.amount || 0), 0),
);
const departmentCostAmount = computed(() =>
  costAllocations.value
    .filter((item) => item.costType === "DEPARTMENT")
    .reduce((sum, item) => sum + Number(item.amount || 0), 0),
);
const filteredCostAllocations = computed(() =>
  costFilter.value === "ALL"
    ? costAllocations.value
    : costAllocations.value.filter(
...
const selectedOrderRequest = computed(
  () =>
    purchaseRequests.value.find((item) => item.id === orderForm.requestId) ||
    null,
);
...
const remainingQuantity = computed(() =>
  Math.max(
    0,
    Number(selectedOrder.value?.orderedQty || 0) -
      Number(selectedOrder.value?.receivedQty || 0),
  ),
);
...
function handleRequestPageChange() {
  loadRequests();
}

async function loadRequests() {
  try {
    const result = await listPurchaseRequests({ page: 0, size: 999 });
    purchaseRequests.value = result.content;
  } catch (e: any) {
    message.error(e instanceof Error ? e.message : "加载采购申请失败");
  }
}
async function loadOrders() {
  try {
    const result = await listPurchaseOrders({ page: 0, size: 999 });
    purchaseOrders.value = result.content;
  } catch (e: any) {
    message.error(e instanceof Error ? e.message : "加载采购订单失败");
  }
}
...
function openOrder() {
  if (!requestOptions.value.length) {
    message.warning("暂无已审批且未下单的采购申请");
    return;
  }
...
function syncRequestPart(partId: string) {
  requestForm.partName =
    parts.value.find((item) => item.id === partId)?.name || "";
}
...
function initialApprovalForm(): ApprovalForm {
  return {
    decision: "APPROVED",
    comment: "同意采购",
...
function receivedPercent(order: PurchaseOrder) {
  return order.orderedQty > 0
    ? Math.min(
        100,
        Math.round(
          (Number(order.receivedQty) / Number(order.orderedQty)) * 100,
        ),
      )
    : 0;
}
function isOverdue(item: ProcurementPayable) {
  return (
    item.status !== "PAID" &&
...
function supplierRiskLabel(status: SupplierRiskStatus) {
  return (
    supplierRiskOptions.find((item) => item.value === status)?.label || status
  );
}
function supplierRiskColor(status: SupplierRiskStatus) {
  return (
    { NORMAL: "green", WATCHLIST: "orange", BLOCKED: "red" } as Record<
      SupplierRiskStatus,
      string
    >
  )[status];
}
function costTypeLabel(type: ProcurementCostType) {
  return type === "PROJECT" ? "项目采购" : "部门采购";
}
function costTypeColor(type: ProcurementCostType) {
  return type === "PROJECT" ? "blue" : "cyan";
}
...
function orderColor(status: PurchaseOrderStatus) {
  return (
    {
      DRAFT: "default",
      ORDERED: "blue",
...
function payableLabel(status: PayableStatus) {
  return (
    {
      PENDING: "待付款",
      PARTIAL_PAID: "部分付款",
...
function payableColor(status: PayableStatus) {
  return (
    {
      PENDING: "orange",
      PARTIAL_PAID: "blue",
