    ...
    <a-card>
      ...
      <a-table :columns="orderColumns" :data-source="orders" :loading="loading" :pagination="{pageSize:10}" :row-key="(r:any)=>r.id" :scroll="{x:1250}">
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.key === 'status'"><a-tag :color="{DRAFT:'default',ORDERED:'blue',PARTIAL_RECEIVED:'orange',RECEIVED:'green',CLOSED:'cyan',CANCELLED:'red'}[record.status]||'default'">{{ {DRAFT:'草稿',ORDERED:'已下单',PARTIAL_RECEIVED:'部分收货',RECEIVED:'已收货',CLOSED:'已关闭',CANCELLED:'已取消'}[record.status]||record.status }}</a-tag></template>
...
onMounted(loadData);
async function loadData(){loading.value=true;try{orders.value=await listPurchaseOrders();}catch(e:any){message.error(e.message||'加载失败');}finally{loading.value=false;}}
