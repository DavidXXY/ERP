  ...
  <div class="page-stack">
    <a-card>
      ...
      <a-space wrap class="table-toolbar">
        <a-button
          ...
          type="primary"
          @click="openCreate"
          ><template #icon><PlusOutlined /></template>新增申请</a-button
        >
      </a-space>
      <a-alert
        class="section-alert"
        type="info"
        show-icon
        message="项目采购会进入项目成本闭环，建议先确认预算余额。"
      />
      <a-table
      ...
      >
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.key === 'material'"
            >{{ record.partName || record.materialName || "-"
            }}<span class="table-subtitle">{{
              record.materialSpec || ""
            }}</span></template
          >
          ...
          <template v-else-if="column.key === 'amount'"
            ><strong>{{
              record.totalAmount || record.unitPrice
                ? formatMoney(
          ...
          <template v-else-if="column.key === 'date'">{{
            record.requiredDate || record.expectedDate || "-"
          }}</template>
          ...
          <template v-else-if="column.key === 'action'">
            <a-space size="small">
              ...
              <a-button
                ...
                >{{
                  record.approvalStatus === "PENDING" ? "审批" : "流程"
                }}</a-button
              >
            </a-space>
          </template>
    ...
    >
      <a-form ref="formRef" :model="form" :rules="rules" layout="vertical">
        <a-row :gutter="16"
          ><a-col :span="12"
            ><a-form-item label="物料名称" name="materialName"
              ><a-input
                v-model:value="form.materialName" /></a-form-item></a-col
          ><a-col :span="12"
            ><a-form-item label="规格型号"
              ><a-input
                v-model:value="form.materialSpec" /></a-form-item></a-col
        ></a-row>
        <a-row :gutter="16"
          ><a-col :span="8"
          ...
          ><a-col :span="8"
            ><a-form-item label="单位"
              ><a-input v-model:value="form.unit" /></a-form-item></a-col
          ><a-col :span="8"
          ...
          ><a-col :span="8"
            ><a-form-item label="成本归属"
              ><a-select
                ...
                @change="form.costTargetId = ''" /></a-form-item></a-col
        ></a-row>
        <a-form-item
        ...
        <a-alert
          type="info"
          show-icon
          :message="`预计采购金额 ${formatMoney(estimatedAmount)}，审批通过后进入请购到下单、到货、应付、付款闭环。`"
        />
      </a-form>
    </a-modal>
    <a-modal
    ...
    >
      <a-alert
        ...
        class="section-alert"
        type="info"
        :message="`${selectedRequest.code || '-'} · ${selectedRequest.partName || selectedRequest.materialName || '-'} · ${formatMoney(selectedRequest.totalAmount || Number(selectedRequest.unitPrice || 0) * Number(selectedRequest.quantity || 0))}`"
      />
      <a-card
        v-if="selectedRequest"
        size="small"
...
const approvalForm = reactive<{
  ...
  comment: string;
  approverName: string;
}>({ decision: "APPROVED", comment: "同意采购", approverName: "" });
const rules = {
...
const selectedTarget = computed(() =>
  (form.costType === "PROJECT" ? projects.value : departments.value).find(
    (item) => item.id === form.costTargetId,
  ),
);
...
const canApproveSelected = computed(
  () =>
    ...
    selectedRequest.value.approvalStatus === "PENDING" &&
    auth.can("procurement:request:approve"),
);
const targetHint = computed(() =>
  form.costType === "PROJECT"
...
async function handleCreate() {
  ...
  try {
    ...
    message.success("申请已创建，并进入采购审批闭环");
    await loadData();
  } catch (e: any) {
...
async function handleSubmit(record: PurchaseRequest) {
  try {
    await processPurchaseRequestApproval(record.id, {
...
function purchaseApprovalSteps(item: PurchaseRequest): ApprovalProgressStep[] {
  ...
  return [
    {
      key: "start",
      personName: item.requesterName || item.applicantName || "发起人",
      title: "发起申请",
      time: item.expectedDate || item.requiredDate,
      note: item.reason || item.description,
      state: "done",
    },
    {
      key: "approval",
    ...
    {
      key: "execute",
      personName: executed ? statusLabel(item.status) : "采购执行",
      title: executed ? "已执行" : "待执行",
      note:
