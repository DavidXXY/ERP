<template>
    ...
    <a-modal :open="costOpen" @update:open="emit('update:costOpen', $event)" title="登记项目成本" width="760px" :confirm-loading="saving" @ok="handleCreateCost">
      ...
      <a-alert
        v-if="detail"
        class="section-alert"
        :type="costBudgetOverrun ? 'error' : costBudgetUsageAfter >= 85 ? 'warning' : 'success'"
        show-icon
        :message="`登记后预算使用率 ${costBudgetUsageAfter.toFixed(1)}%，预计余额 ${formatMoney(projectedBudgetVariance)}`"
        :description="costBudgetOverrun ? '该成本会导致项目超预算，保存时将被后端预算控制拦截，请先走预算调整或审批。' : '成本登记后会同步刷新项目实际成本和毛利。'"
      />
...
async function handleCreateCost() {
  ...
  try {
    ...
    message.error(msg.includes("Cost exceeds budget") ? "成本超过项目预算，已被预算控制拦截" : msg);
  }
}
</script>
