<template>
  <div class="page-stack self-approval-page">
    <a-card title="我的待办审批" :bordered="false" class="approval-hero-card">
      <template #extra>
        <a-space wrap class="approval-toolbar">
          <a-select v-model:value="riskFilter" allow-clear placeholder="风险筛选" :options="riskOptions" class="risk-filter" />
      ...
      <a-row :gutter="[16, 16]" class="self-metric-grid">
        <a-col :xs="12" :lg="6"><div class="self-metric-card"><span>待处理</span><strong>{{ pendingApprovals.length }}</strong></div></a-col>
        <a-col :xs="12" :lg="6"><div class="self-metric-card"><span>待审金额</span><strong>{{ formatCompactMoney(pendingAmount) }}</strong></div></a-col>
        <a-col :xs="12" :lg="6"><div class="self-metric-card danger"><span>超时</span><strong>{{ overdueCount }}</strong></div></a-col>
        <a-col :xs="12" :lg="6"><div class="self-metric-card danger"><span>高风险</span><strong>{{ highRiskCount }}</strong></div></a-col>
      </a-row>
    </a-card>
      ...
      >
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.key === 'status'">
            ...
            <a-tag :color="riskColor(riskLevel(record))">{{ riskLabel(riskLevel(record)) }}</a-tag>
            <span v-if="record.currentApproverName" class="table-subtitle">当前：{{ record.currentApproverName }}</span>
          </template>
          <template v-else-if="column.key === 'date'">
            {{ record.createdAt?.slice(0, 10) || '-' }}
            <span class="table-subtitle">{{ approvalAgeLabel(record.createdAt) }}</span>
          </template>
          <template v-else-if="column.key === 'action'">
            <a-button type="link" size="small" @click="openProcess(record, 'APPROVED')">通过</a-button>
            <a-button type="link" size="small" danger @click="openProcess(record, 'REJECTED')">驳回</a-button>
          </template>
        </template>
        <template #emptyText>
    ...
    <a-modal v-model:open="processOpen" :title="processForm.decision === 'APPROVED' ? '审批通过' : '审批驳回'" :confirm-loading="saving" @ok="handleProcess">
      <a-alert
      ...
      <a-form ref="processFormRef" :model="processForm" :rules="processRules" layout="vertical">
        <a-form-item label="审批结论" name="decision">
          <a-radio-group v-model:value="processForm.decision" button-style="solid">
...
const processRules = { decision: [{ required: true }], comment: [{ required: true, message: "请填写审批意见" }], approverName: [{ required: true, message: "请输入审批人" }] };
const riskOptions = [{ label: "高风险", value: "HIGH" }, { label: "中风险", value: "MEDIUM" }, { label: "正常", value: "NORMAL" }];
const columns = [
...
const filteredApprovals = computed(() => pendingApprovals.value.filter((item) => !riskFilter.value || riskLevel(item) === riskFilter.value));
const pendingAmount = computed(() => pendingApprovals.value.reduce((sum, item) => sum + Number(item.amount || 0), 0));
const overdueCount = computed(() => pendingApprovals.value.filter((item) => approvalAgeHours(item.createdAt) >= 48).length);
const highRiskCount = computed(() => pendingApprovals.value.filter((item) => riskLevel(item) === "HIGH").length);

onMounted(loadData);

async function loadData() {
  loading.value = true;
  try { approvals.value = await getSelfApprovals(); }
  catch (error) { message.error(error instanceof Error ? error.message : "待办审批加载失败"); }
  finally { loading.value = false; }
}

function openProcess(record: Approval, decision: "APPROVED" | "REJECTED") {
  selectedApproval.value = record;
  Object.assign(processForm, { decision, comment: decision === "APPROVED" ? "同意" : "", approverName: auth.user?.displayName || "" });
...
async function handleProcess() {
  ...
  try {
    ...
    message.success(processForm.decision === "APPROVED" ? "审批已通过" : "审批已驳回");
    await loadData();
  } catch (error) {
...
function riskLabel(value: string) { return ({ HIGH: "高风险", MEDIUM: "中风险", NORMAL: "正常" } as Record<string, string>)[value] || value; }
function riskColor(value: string) { return ({ HIGH: "red", MEDIUM: "orange", NORMAL: "green" } as Record<string, string>)[value] || "default"; }
function approvalTypeLabel(value: ApprovalType) { return ({ QUOTE: "报价", CONTRACT: "合同", PURCHASE: "采购", OUTSOURCE: "外包", EXPENSE: "报销", PAYMENT: "付款", SEAL: "用章", LEAVE: "请假", TRAVEL: "出差", OTHER: "其他" } as Record<ApprovalType, string>)[value] || value; }
function formatMoney(value: number) { return new Intl.NumberFormat("zh-CN", { style: "currency", currency: "CNY", minimumFractionDigits: 2 }).format(value || 0); }
...
.approval-hero-card { background: #f8fafc; }
.approval-table-card {
  margin-top: 0;
}
