  ...
  <div class="page-stack self-page">
    <a-card :bordered="false" class="self-hero-card">
      <div class="self-hero-main">
        <div>
          <a-typography-title :level="3">{{ greeting }}，{{ employee?.name || '加载中' }}</a-typography-title>
        ...
        <a-space wrap class="self-hero-actions">
          <a-button @click="$router.push('/self/approvals')"><template #icon><CheckSquareOutlined /></template>待办审批</a-button>
      ...
      <a-row :gutter="[16, 16]" class="self-metric-grid">
        <a-col :xs="12" :lg="6"><div class="self-metric-card"><span>待办事项</span><strong>{{ todos.length }}</strong></div></a-col>
        <a-col :xs="12" :lg="6"><div class="self-metric-card warn"><span>待办审批</span><strong>{{ approvalTodoCount }}</strong></div></a-col>
        <a-col :xs="12" :lg="6"><div class="self-metric-card"><span>本月新增</span><strong>{{ analytics.newThisMonth }}</strong></div></a-col>
        <a-col :xs="12" :lg="6"><div class="self-metric-card warn"><span>待审批请假</span><strong>{{ analytics.leavePendingCount }}</strong></div></a-col>
      </a-row>
    </a-card>
    ...
    <a-row :gutter="[16, 16]">
      <a-col :xs="24" :lg="15">
        <a-card title="待办事项" class="self-work-card">
          <template #extra>
            <a-button type="link" @click="$router.push('/self/approvals')">待办审批 {{ approvalTodoCount }}</a-button>
          </template>
      ...
      <a-col :xs="24" :lg="9">
        <a-card title="快捷操作" class="quick-actions self-work-card">
          <a-row :gutter="[10, 10]">
            ...
            <a-col :xs="12"><a-button block @click="$router.push('/self/approvals')"><template #icon><CheckSquareOutlined /></template>待办审批</a-button></a-col>
      ...
      <a-col :xs="24" :lg="16">
        <a-card title="我的请假额度">
          ...
          <a-row :gutter="16" v-if="balances.length">
            <a-col :xs="24" :sm="12" v-for="b in balances.slice(0, 4)" :key="b.id">
      ...
      <a-col :xs="24" :lg="8">
        <a-card title="员工概览">
          <a-descriptions :column="1" size="small">
            <a-descriptions-item label="总人数">{{ analytics.totalEmployees }}</a-descriptions-item>
            <a-descriptions-item label="在职人数">{{ analytics.activeEmployees }}</a-descriptions-item>
            <a-descriptions-item label="当前组织">{{ employee?.organizationName || '-' }}</a-descriptions-item>
          </a-descriptions>
        </a-card>
      </a-col>
    </a-row>
    ...
    <a-card title="最近请假记录" class="recent-leaves">
      ...
      <a-table :data-source="recentLeaves" :columns="leafCols" size="small" row-key="id" :pagination="{ pageSize: 5 }" :loading="loading" :scroll="{ x: 760 }">
...
import { PlusOutlined, OrderedListOutlined, UserOutlined, BarChartOutlined, CheckSquareOutlined } from "@ant-design/icons-vue";
...
const approvalTodoCount = computed(() => todos.value.filter((item) => item.type === "APPROVAL_PENDING").length);
...
.self-hero-card { background: #f8fafc; }
.self-hero-main { display: flex; justify-content: space-between; align-items: flex-start; gap: 16px; }
.self-hero-main h3 { margin: 0 0 6px; }
.self-hero-main :deep(.ant-typography-secondary),
.self-hero-main :deep(.ant-typography) { color: #64748b; }
.self-hero-actions { flex: none; }
.self-work-card { height: 100%; }
.recent-leaves { margin-top: 0; }
...
@media (max-width: 640px) {
  .self-hero-main { display: grid; }
  .self-hero-actions,
  .self-hero-actions :deep(.ant-space-item),
  .self-hero-actions :deep(.ant-btn) { width: 100%; }
  .todo-card :deep(.ant-list-item) { align-items: flex-start; }
  .todo-card :deep(.ant-list-item-action) { margin-left: 8px; }
  .balance-header { gap: 8px; }
}
</style>
