    ...
    <a-card title="提交请假">
      ...
      <a-form ref="formRef" :model="form" layout="vertical">
        ...
        <div v-if="selectedTypeBalance" class="balance-hint">
          <a-alert
            :message="`${typeLabel(form.leaveType)}剩余额度：${selectedTypeBalance.remainingDays.toFixed(1)} 天`"
            type="info"
            show-icon
          />
        </div>
        <a-form-item class="form-actions">
          <a-button type="primary" :loading="saving" @click="submitLeave"
            >提交申请</a-button
          >
...
const form = reactive<LeavePayload>({
  leaveType: "ANNUAL",
  startDate: "",
  endDate: "",
  totalDays: 1,
...
const selectedTypeBalance = computed(() =>
  balances.value.find((b) => b.leaveType === form.leaveType),
);
...
function onTypeChange() {
  if (
    selectedTypeBalance.value &&
    form.totalDays > selectedTypeBalance.value.remainingDays
  ) {
    message.warning(
      `当前${typeLabel(form.leaveType)}余额不足（剩余 ${selectedTypeBalance.value.remainingDays.toFixed(1)} 天）`,
    );
  }
}
