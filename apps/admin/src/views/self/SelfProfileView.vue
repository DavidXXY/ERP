      ...
      <template v-else-if="detail">
        <section class="profile-summary">
          <div class="profile-avatar">
            {{ detail.employee.name?.slice(0, 1) || "我" }}
          </div>
          <div>
            <strong>{{ detail.employee.name }}</strong>
            <span
              >{{ detail.employee.workNo || "未登记工号" }} ·
              {{ detail.employee.position || "未设置岗位" }}</span
            >
          </div>
          <a-tag
            v-if="detail.employee.contractEnd"
            :color="contractTagColor(detail.employee.contractEnd)"
            >{{ contractTagLabel(detail.employee.contractEnd) }}</a-tag
          >
        </section>
        ...
        <a-descriptions bordered :column="{ xs: 1, sm: 1, md: 2 }" size="small">
          ...
          <a-descriptions-item label="工号">{{
            detail.employee.workNo || "-"
          }}</a-descriptions-item>
          <a-descriptions-item label="部门">{{
            detail.employee.organizationName || "-"
          }}</a-descriptions-item>
          ...
          <a-descriptions-item label="身份证号">{{
            detail.employee.idCard || "-"
          }}</a-descriptions-item>
          <a-descriptions-item label="入职日期">{{
            detail.employee.entryDate || "-"
          }}</a-descriptions-item>
...
async function loadData() {
  loading.value = true;
  try {
    detail.value = await getSelfProfile();
  } catch (error: any) {
