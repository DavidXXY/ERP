  ...
  <div class="page-stack approval-config-page">
        ...
        <a-collapse-panel v-for="group in groupedFlows" :key="group.flowCode">
          <template #header>
            <div class="flow-panel-title">
              ...
              <span v-if="group.versionNo">当前版本 V{{ group.versionNo }}</span>
              <div v-if="flowWarnings(group).length" class="flow-warnings">
                <a-tag v-for="warning in flowWarnings(group)" :key="warning" color="orange">{{ warning }}</a-tag>
              </div>
            </div>
          </template>
          <template #extra>
            <a-space @click.stop>
              <a-button size="small" type="link" @click="publishFlow(group.flowCode)">发布</a-button>
              <a-button size="small" type="link" @click="openVersions(group.flowCode)">版本</a-button>
              <a-button size="small" type="link" @click="openAdd(group.flowCode)">新增规则</a-button>
            </a-space>
          </template>
          <div class="step-stack">
            <section v-for="step in group.steps" :key="step.stepNo" class="step-section">
              <div class="step-title">
                <a-tag color="blue">第 {{ step.stepNo }} 步</a-tag>
                <span>{{ step.items.length }} 条规则，{{ step.items.filter((item: ApprovalConfigResponse) => item.enabled).length }} 条启用</span>
              </div>
              <a-table
              ...
              >
                <template #bodyCell="{ column, record }">
                  <template v-if="column.key === 'assignee'">
                    <a-tag :color="assigneeTypeColor(record.assigneeType)">{{ assigneeTypeLabel(record.assigneeType) }}</a-tag>
                    {{ record.assigneeName }}
                    <a-tag v-if="record.assigneeType === 'ROLE' && roleUserCount(record.roleId) === 0" color="red">无成员</a-tag>
                    <a-tag v-if="!record.enabled" color="default">停用</a-tag>
                    <a-tag>V{{ record.versionNo }}</a-tag>
                  </template>
                  <template v-else-if="column.key === 'mode'"><a-tag :color="record.approvalMode === 'SEQUENTIAL' ? 'blue' : 'green'">{{ record.approvalMode === 'SEQUENTIAL' ? "依次" : "同步" }}</a-tag></template>
                  <template v-else-if="column.key === 'condition'">
                    ...
                    <span v-if="record.slaHours" class="table-subtitle">SLA {{ record.slaHours }} 小时</span>
                  </template>
                  <template v-else-if="column.key === 'action'">
                    <a-space>
                      <a-button type="link" @click.stop="openEdit(record)">编辑</a-button>
                      <a-popconfirm title="确定移除该审批规则？" @confirm="removeConfig(record.id)">
          ...
          <div class="preview-panel">
            ...
            <a-row :gutter="12">
              <a-col :xs="24" :md="6"><a-input-number v-model:value="previewForms[group.flowCode].amount" placeholder="金额/天数" :min="0" :precision="2" style="width:100%" /></a-col>
              <a-col :xs="24" :md="6"><a-input v-model:value="previewForms[group.flowCode].departmentName" placeholder="部门/组织" /></a-col>
              <a-col :xs="24" :md="6"><a-input v-model:value="previewForms[group.flowCode].businessType" placeholder="业务类型" /></a-col>
              <a-col :xs="24" :md="6"><a-button block @click="runPreview(group.flowCode)">预览路径</a-button></a-col>
            </a-row>
            <a-row :gutter="12" style="margin-top: 10px">
              <a-col :xs="24" :md="6"><a-input v-model:value="previewForms[group.flowCode].projectCode" placeholder="项目编码" /></a-col>
              <a-col :xs="24" :md="6"><a-select v-model:value="previewForms[group.flowCode].supplierRisk" allow-clear placeholder="供应商风险" :options="supplierRiskOptions" style="width:100%" /></a-col>
              <a-col :xs="24" :md="6"><a-input v-model:value="previewForms[group.flowCode].customerLevel" placeholder="客户等级" /></a-col>
            </a-row>
            <div v-if="previewResults[group.flowCode]" class="preview-result">
              <a-alert :message="`${previewResults[group.flowCode]?.ruleText} · V${previewResults[group.flowCode]?.versionNo}`" type="success" show-icon />
              <a-steps :current="-1" size="small" class="preview-steps">
                <a-step v-for="step in previewResults[group.flowCode]?.steps" :key="step.stepNo" :title="`第 ${step.stepNo} 步`" :description="stepDescription(step)" />
              </a-steps>
            </div>
          </div>
        </a-collapse-panel>
    ...
    <a-modal v-model:open="addOpen" :title="editingId ? '编辑审批规则' : '新增审批规则'" :confirm-loading="saving" @ok="saveConfig">
      <a-form ref="formRef" :model="form" :rules="rules" layout="vertical">
        ...
        <a-form-item v-if="form.assigneeType === 'ROLE' || form.assigneeType === 'USER'" :label="form.assigneeType === 'ROLE' ? '审批角色' : '审批人员'" name="targetIds">
          <a-select
        ...
        <a-form-item v-else-if="form.assigneeType === 'DYNAMIC'" label="动态审批对象" name="dynamicAssignee">
          <a-select v-model:value="form.dynamicAssignee" :options="dynamicAssigneeOptions" />
        </a-form-item>
        <a-alert v-else type="warning" show-icon message="命中该规则时将自动通过，请确认条件足够明确。" style="margin-bottom: 16px" />
        ...
        <a-form-item label="节点通过策略">
          <a-select v-model:value="form.stepPolicy" :options="stepPolicyOptions" />
        </a-form-item>
        <a-row :gutter="12">
          <a-col :span="12"><a-form-item label="SLA小时数"><a-input-number v-model:value="form.slaHours" :min="1" :precision="0" style="width:100%" /></a-form-item></a-col>
          <a-col :span="12"><a-form-item label="超时升级角色"><a-select v-model:value="form.escalationRoleId" allow-clear show-search option-filter-prop="label" :options="roleOptions" /></a-form-item></a-col>
        </a-row>
    ...
    <a-modal v-model:open="copyOpen" title="复制审批流" :confirm-loading="saving" @ok="handleCopyFlow">
      <a-form :model="copyForm" layout="vertical">
        <a-form-item label="源审批流"><a-select v-model:value="copyForm.sourceFlowCode" :options="flowOptions" /></a-form-item>
        <a-form-item label="目标审批流"><a-select v-model:value="copyForm.targetFlowCode" :options="flowOptions" /></a-form-item>
        <a-form-item><a-checkbox v-model:checked="copyForm.overwrite">覆盖目标审批流当前规则</a-checkbox></a-form-item>
      </a-form>
    </a-modal>

    <a-modal v-model:open="batchOpen" title="批量模拟审批路径" width="760px" :confirm-loading="saving" @ok="runBatchPreview">
      <a-textarea v-model:value="batchText" :rows="8" />
      <div v-if="batchResults.length" class="batch-results">
        <a-alert v-for="(item, index) in batchResults" :key="index" type="success" show-icon :message="`${item.flowCode} · V${item.versionNo} · ${item.ruleText}`" :description="item.steps.map(stepDescription).join(' / ')" />
      </div>
    </a-modal>

    <a-modal v-model:open="versionOpen" title="审批流版本" :footer="null">
      <a-table size="small" :data-source="versions" :columns="versionColumns" :pagination="false" row-key="versionNo">
        <template #bodyCell="{ column, record }">
          <template v-if="column.key === 'action'">
            <a-popconfirm title="确认回滚到该版本？" @confirm="rollbackFlow(record.versionNo)">
...
import { batchPreviewApprovalFlows, copyApprovalFlow, createApprovalConfig, deleteApprovalConfig, listApprovalConfigs, listApprovalDiagnostics, listApprovalFlowVersions, listRolesApi, listUsersApi, previewApprovalFlow, publishApprovalFlow, rollbackApprovalFlow, updateApprovalConfig, type ApprovalConfigResponse, type ApprovalFlowPreview, type ApprovalFlowVersion, type RoleResponse, type UserResponse } from "@/api/system";
...
const configs = ref<ApprovalConfigResponse[]>([]); const users = ref<UserResponse[]>([]); const roles = ref<RoleResponse[]>([]); const versions = ref<ApprovalFlowVersion[]>([]); const selectedVersionFlow = ref("QUOTE"); const loading = ref(false); const saving = ref(false); const addOpen = ref(false); const copyOpen = ref(false); const batchOpen = ref(false); const versionOpen = ref(false); const editingId = ref<string | null>(null); const activeFlowKeys = ref<string[]>([]); const formRef = ref<FormInstance>();
const previewResults = ref<Record<string, ApprovalFlowPreview | undefined>>({});
const batchResults = ref<ApprovalFlowPreview[]>([]);
const previewForms = reactive<Record<string, { amount?: number; departmentName: string; businessType: string; projectCode: string; supplierRisk?: string; customerLevel: string }>>({});
const form = reactive({ flowCode: "QUOTE", assigneeType: "ROLE" as ApprovalConfigResponse["assigneeType"], targetIds: [] as string[], dynamicAssignee: "DEPARTMENT_LEADER", approvalMode: "SEQUENTIAL" as "PARALLEL" | "SEQUENTIAL", sequenceNo: 1, conditionType: "ANY" as ApprovalConfigResponse["conditionType"], minAmount: undefined as number | undefined, maxAmount: undefined as number | undefined, departmentName: "", businessType: "", projectCode: "", supplierRisk: undefined as string | undefined, customerLevel: "", priority: 100, stepPolicy: "ANY_APPROVE" as ApprovalConfigResponse["stepPolicy"], slaHours: undefined as number | undefined, escalationRoleId: undefined as string | undefined, remark: "", enabled: true });
const copyForm = reactive({ sourceFlowCode: "QUOTE", targetFlowCode: "CONTRACT", overwrite: false });
const batchText = ref(JSON.stringify([{ flowCode: "QUOTE", amount: 60000, customerLevel: "VIP" }, { flowCode: "EXPENSE", amount: 1200, businessType: "TRAVEL" }], null, 2));
...
const stepPolicyOptions = [{ label: "任一通过", value: "ANY_APPROVE" }, { label: "全部通过", value: "ALL_APPROVE" }, { label: "多数通过", value: "MAJORITY_APPROVE" }];
...
const versionColumns = [{ title: "版本", dataIndex: "versionNo" }, { title: "规则数", dataIndex: "ruleCount" }, { title: "状态", dataIndex: "publishStatus" }, { title: "操作", key: "action", width: 100 }];
...
const groupedFlows = computed(() => flowOptions.map(flow => {
  ...
  const stepNos = [...new Set(items.map(item => item.sequenceNo))].sort((a, b) => a - b);
  return { flowCode: flow.value, flowName: flow.flowName, items, versionNo: items.reduce((max, item) => Math.max(max, item.versionNo || 1), 0), maxStep: items.reduce((max, item) => Math.max(max, item.sequenceNo), 0), steps: stepNos.map(stepNo => ({ stepNo, items: items.filter(item => item.sequenceNo === stepNo) })) };
}));

flowOptions.forEach(item => { previewForms[item.value] = { departmentName: "", businessType: "", projectCode: "", customerLevel: "" }; });

async function loadData() { loading.value = true; try { const [configData, userData, roleData, diagnostics] = await Promise.all([listApprovalConfigs(), listUsersApi(0, 200), listRolesApi(0, 200), listApprovalDiagnostics()]); configs.value = configData; users.value = userData.content; roles.value = roleData.content; if (diagnostics.some(item => item.severity === "HIGH")) message.warning("审批流存在高风险配置，请查看流程面板提示"); if (!activeFlowKeys.value.length) activeFlowKeys.value = flowOptions.map(item => item.value); } catch (error) { message.error(error instanceof Error ? error.message : "审批配置加载失败"); } finally { loading.value = false; } }
function openAdd(flowCode = "QUOTE") { editingId.value = null; Object.assign(form, { flowCode, assigneeType: "ROLE", targetIds: [], dynamicAssignee: "DEPARTMENT_LEADER", approvalMode: "SEQUENTIAL", sequenceNo: 1, conditionType: "ANY", minAmount: undefined, maxAmount: undefined, departmentName: "", businessType: "", projectCode: "", supplierRisk: undefined, customerLevel: "", priority: 100, stepPolicy: "ANY_APPROVE", slaHours: undefined, escalationRoleId: undefined, remark: "", enabled: true }); addOpen.value = true; }
function openEdit(record: ApprovalConfigResponse) {
  editingId.value = record.id;
  Object.assign(form, {
    flowCode: record.flowCode,
    assigneeType: record.assigneeType,
    targetIds: [record.assigneeType === "ROLE" ? record.roleId : record.assigneeType === "USER" ? record.userId : undefined].filter(Boolean) as string[],
...
async function saveConfig() {
  ...
  try {
    ...
    const uniqueTargetIds = form.assigneeType === "USER" || form.assigneeType === "ROLE" ? [...new Set(form.targetIds)] : ["__virtual__"];
    if (editingId.value) {
      ...
      await updateApprovalConfig(editingId.value, {
        ...
        dynamicAssignee: form.assigneeType === "DYNAMIC" ? form.dynamicAssignee : undefined,
      ...
      message.success(`已配置 ${form.assigneeType === "USER" || form.assigneeType === "ROLE" ? uniqueTargetIds.length : 1} 个审批对象`);
    }
    addOpen.value = false;
    await loadData();
  } catch (error) {
...
async function publishFlow(flowCode: string) {
  try { await publishApprovalFlow(flowCode); message.success("审批流已发布"); await loadData(); }
...
async function rollbackFlow(versionNo: number) {
  try { await rollbackApprovalFlow(selectedVersionFlow.value, versionNo); message.success("已回滚并生成新版本"); versionOpen.value = false; await loadData(); }
  catch (error) { message.error(error instanceof Error ? error.message : "回滚失败"); }
}
function openCopyFlow() { Object.assign(copyForm, { sourceFlowCode: "QUOTE", targetFlowCode: "CONTRACT", overwrite: false }); copyOpen.value = true; }
async function handleCopyFlow() {
  const target = flowOptions.find(item => item.value === copyForm.targetFlowCode)!;
  saving.value = true;
  try {
    await copyApprovalFlow({ sourceFlowCode: copyForm.sourceFlowCode, targetFlowCode: copyForm.targetFlowCode, targetFlowName: target.flowName, overwrite: copyForm.overwrite });
    copyOpen.value = false; message.success("审批流已复制"); await loadData();
  } catch (error) { message.error(error instanceof Error ? error.message : "复制失败"); }
  finally { saving.value = false; }
}
function openBatchPreview() { batchResults.value = []; batchOpen.value = true; }
async function runBatchPreview() {
  ...
  } catch (error) { message.error(error instanceof Error ? error.message : "批量模拟失败"); }
  finally { saving.value = false; }
}
...
async function runPreview(flowCode: string) {
  try {
    previewResults.value[flowCode] = await previewApprovalFlow({ flowCode, ...previewForms[flowCode] });
  } catch (error) {
...
function flowWarnings(group: { items: ApprovalConfigResponse[]; steps: Array<{ stepNo: number; items: ApprovalConfigResponse[] }> }) {
  const warnings: string[] = [];
  const enabledItems = group.items.filter(item => item.enabled);
  if (!enabledItems.length) warnings.push("无启用规则");
  if (!enabledItems.some(item => item.sequenceNo === 1 && item.conditionType === "ANY")) warnings.push("缺少默认第1步");
  if (enabledItems.some(item => item.assigneeType === "ROLE" && roleUserCount(item.roleId) === 0)) warnings.push("存在空角色");
  if (enabledItems.some(item => item.assigneeType === "AUTO" && item.conditionType === "ANY")) warnings.push("自动通过过宽");
  if (hasAmountOverlap(group.steps)) warnings.push("金额区间重叠");
  return warnings;
}
function hasAmountOverlap(steps: Array<{ stepNo: number; items: ApprovalConfigResponse[] }>) {
  return steps.some(step => {
    const ranges = step.items
      .filter(item => item.enabled && (item.conditionType === "AMOUNT" || item.conditionType === "AMOUNT_AND_DEPARTMENT"))
      .map(item => ({ start: item.minAmount ?? 0, end: item.maxAmount ?? Number.POSITIVE_INFINITY }))
...
function assigneeTypeLabel(value: ApprovalConfigResponse["assigneeType"]) { return ({ ROLE: "角色", USER: "人员", DYNAMIC: "动态", AUTO: "自动" } as Record<string, string>)[value] || value; }
function assigneeTypeColor(value: ApprovalConfigResponse["assigneeType"]) { return ({ ROLE: "purple", USER: "blue", DYNAMIC: "cyan", AUTO: "green" } as Record<string, string>)[value] || "default"; }
...
function conditionText(record: ApprovalConfigResponse) {
  ...
  if (record.escalationRoleName) parts.push(`升级 ${record.escalationRoleName}`);
