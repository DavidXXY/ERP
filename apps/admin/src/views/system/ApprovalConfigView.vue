<template>
  <div class="page-stack approval-config-page">
    <a-card>
      ...
      <a-alert type="info" show-icon message="并行审批：任一配置人员通过即可；依次审批：按顺序逐级通过。未配置人员的流程保持原有权限规则。" style="margin-bottom: 16px" />
    ...
    <a-modal v-model:open="addOpen" title="新增审批人员" :confirm-loading="saving" @ok="saveConfig">
      <a-form ref="formRef" :model="form" :rules="rules" layout="vertical">
        ...
        <a-form-item label="审批人员" name="userId"><a-select v-model:value="form.userId" show-search option-filter-prop="label" :options="userOptions" /></a-form-item>
        ...
        <a-form-item v-if="form.approvalMode === 'SEQUENTIAL'" label="审批顺序" name="sequenceNo"><a-input-number v-model:value="form.sequenceNo" :min="1" :precision="0" /></a-form-item>
...
const rules = { flowCode: [{ required: true, message: "请选择审批流程" }], userId: [{ required: true, message: "请选择审批人员" }], approvalMode: [{ required: true }], sequenceNo: [{ required: true, message: "请输入审批顺序" }] };
...
function openAdd() { Object.assign(form, { flowCode: "QUOTE", userId: "", approvalMode: "PARALLEL", sequenceNo: 1, conditionType: "ANY", minAmount: undefined, maxAmount: undefined, departmentName: "", businessType: "", projectCode: "", supplierRisk: undefined, customerLevel: "", priority: 100, remark: "" }); addOpen.value = true; }
async function saveConfig() { await formRef.value?.validate(); saving.value = true; try { const flow = flowOptions.find(item => item.value === form.flowCode)!; await createApprovalConfig({ flowCode: flow.value, flowName: flow.flowName, userId: form.userId, approvalMode: form.approvalMode, sequenceNo: form.sequenceNo, conditionType: form.conditionType, minAmount: form.minAmount, maxAmount: form.maxAmount, departmentName: form.departmentName, businessType: form.businessType, projectCode: form.projectCode, supplierRisk: form.supplierRisk, customerLevel: form.customerLevel, priority: form.priority, remark: form.remark }); addOpen.value = false; message.success("审批节点已配置"); await loadData(); } catch (error) { message.error(error instanceof Error ? error.message : "配置保存失败"); } finally { saving.value = false; } }
