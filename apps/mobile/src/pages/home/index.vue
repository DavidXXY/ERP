...
const metricCount = computed(() =>
  [capabilities.value?.approvals, capabilities.value?.workOrders, capabilities.value?.notifications].filter(Boolean).length || 1,
);
  ...
  <view>
    ...
    <view class="page-shell home-shell">
      <StateView :loading="loading" :error="error" @retry="load">
        <view class="metric-strip surface" :style="{ gridTemplateColumns: `repeat(${metricCount}, 1fr)` }">
          <view v-if="capabilities?.approvals" class="metric" @click="uni.switchTab({ url: '/pages/approvals/index' })"><text class="metric-value">{{ data?.pendingApprovals || 0 }}</text><text>待审批</text></view>
          <view v-if="capabilities?.workOrders" class="metric" @click="uni.navigateTo({ url: '/pages/work-orders/index' })"><text class="metric-value">{{ data?.activeWorkOrders || 0 }}</text><text>进行中</text></view>
          <view v-if="capabilities?.notifications" class="metric" @click="uni.navigateTo({ url: '/pages/notifications/index' })"><text class="metric-value danger-value">{{ data?.unreadNotifications || 0 }}</text><text>未读消息</text></view>
        </view>

        <view v-if="capabilities?.offlineOperations && offlineCount" class="offline-banner" @click="uni.navigateTo({ url: '/pages/offline/index' })">
        ...
        <view class="quick-grid">
          <view v-if="capabilities?.approvals" class="quick-item" @click="uni.switchTab({ url: '/pages/approvals/index' })"><view class="quick-icon green"><uni-icons type="checkbox" size="24" color="#176b5b" /></view><text>审批处理</text></view>
          <view v-if="capabilities?.workOrders" class="quick-item" @click="uni.navigateTo({ url: '/pages/work-orders/index' })"><view class="quick-icon dark"><uni-icons type="gear" size="24" color="#fff" /></view><text>现场工单</text></view>
          <view v-if="capabilities?.leaveApplication || capabilities?.expenseApplication || capabilities?.travelApplication" class="quick-item" @click="uni.switchTab({ url: '/pages/applications/index' })"><view class="quick-icon amber"><uni-icons type="compose" size="24" color="#9a5c0d" /></view><text>发起申请</text></view>
          <view v-if="capabilities?.notifications" class="quick-item" @click="uni.navigateTo({ url: '/pages/notifications/index' })"><view class="quick-icon red"><uni-icons type="notification" size="24" color="#b53232" /></view><text>消息通知</text></view>
        </view>

        <template v-if="capabilities?.workOrders">
        <view class="section-head row-between"><text class="section-title">今日任务轨道</text><text class="section-link" @click="uni.navigateTo({ url: '/pages/work-orders/index' })">全部工单</text></view>
...
.metric-strip { display: grid; padding: 26rpx 0; }
...
.quick-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(120rpx, 1fr)); gap: 12rpx; }
