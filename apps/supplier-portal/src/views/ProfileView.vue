...
const expiryWarnings = computed(() => {
  ...
  const check = (label: string, value?: string) => {
    ...
    const days = Math.ceil(
      (new Date(`${value}T23:59:59`).getTime() - today.getTime()) / 86400000,
    );
    if (days < 0)
      warnings.push(`${label}已于 ${value} 到期，请尽快更新资质文件`);
    else if (days <= 90)
      warnings.push(`${label}将于 ${value} 到期（剩 ${days} 天），请提前更新`);
  };
  check("营业执照", profile.licenseValidTo);
  check("资质", profile.qualificationValidTo);
  return warnings;
});
onMounted(load);
async function load() {
  loading.value = true;
  try {
    const session = store.session || (await api.getSession());
