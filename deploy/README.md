...
全新空库首先执行 `B77__fresh_install_baseline.sql`，一次性创建截至 V77 的完整结构和基础权限配置，随后继续执行 V78 之后的增量迁移。当前增量版本为 V87。

已有 V77 数据库会跳过 B77 基线并继续执行增量迁移。应用默认仅忽略已合并且不再随包发布的历史迁移缺失记录，仍校验当前发布迁移的顺序和校验和。升级前必须备份数据库，并先在预发布副本验证 V78 至 V87；不要关闭 Flyway 当前迁移校验。
