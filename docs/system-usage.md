...
用于登记费用申请、费用明细、发票附件、申请人和业务说明。报销单提交后进入审批；审批通过后，拥有付款执行权限的财务人员登记付款日期和银行流水号，系统将报销单更新为已付款并自动生成费用付款凭证。
...
运行环境统一使用 PostgreSQL，并通过 Flyway 管理结构迁移。迁移文件位于 `services/api/src/main/resources/db/migration`。当前增量迁移到 V87；H2 仅作为自动化测试的内存数据库，不提供开发运行入口。
