...
供应商类别由采购方维护，供应商主档中的类别只能从字典选择，供应商门户不能自行填写或修改。采购管理员或采购经理在供应商页面点击“供应商类别字典”即可维护类别名称、说明、排序和启用状态。系统预置以下常用类别：原材料与辅料、生产设备与机械、电气设备与自动化、仪器仪表、IT软硬件与服务、工程施工、维修维保、劳务与外包、物流运输、办公用品与行政物资、包装与耗材、咨询与专业服务、安全环保、能源与公用事业、其他。停用类别不会删除历史供应商数据；已被供应商使用的类别不能改名，如需调整请停用旧类别并新增类别。
