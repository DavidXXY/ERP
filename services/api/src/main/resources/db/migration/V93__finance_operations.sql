CREATE TABLE fin_period_end_jobs (
    ...
    CONSTRAINT uk_period_job_key UNIQUE (tenant_id, idempotency_key),
    CONSTRAINT ck_period_job_period CHECK (period_no BETWEEN 1 AND 12),
    CONSTRAINT ck_period_job_amount CHECK (amount > 0),
    CONSTRAINT ck_period_job_type CHECK (process_type IN ('ACCRUAL','AMORTIZATION','DEPRECIATION','PROFIT_CARRY_FORWARD')),
...
CREATE TABLE fin_partner_reconciliations (
    ...
    CONSTRAINT ck_partner_recon_type CHECK (partner_type IN ('CUSTOMER','SUPPLIER')),
    CONSTRAINT ck_partner_recon_status CHECK (status IN ('PENDING','MATCHED','DISPUTED','CONFIRMED'))
);
...
CREATE TABLE fin_cash_forecast_scenarios (
    ...
    expected_receipts numeric(14,2) NOT NULL,
    expected_payments numeric(14,2) NOT NULL,
    receipt_adjustment numeric(14,2) DEFAULT 0 NOT NULL,
    payment_adjustment numeric(14,2) DEFAULT 0 NOT NULL,
    ...
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    created_by varchar(64), updated_by varchar(64), version bigint DEFAULT 0 NOT NULL,
    CONSTRAINT ck_cash_scenario_horizon CHECK (horizon_days BETWEEN 1 AND 3660),
    CONSTRAINT ck_cash_scenario_status CHECK (status IN ('DRAFT','APPROVED','ARCHIVED'))
);
...
CREATE TABLE fin_tax_filings (
    ...
    input_tax numeric(14,2) NOT NULL,
    tax_payable numeric(14,2) NOT NULL,
    ledger_tax numeric(14,2) NOT NULL,
    difference_amount numeric(14,2) NOT NULL,
    status varchar(24) DEFAULT 'DRAFT' NOT NULL,
    ...
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    created_by varchar(64), updated_by varchar(64), version bigint DEFAULT 0 NOT NULL,
    CONSTRAINT uk_tax_filing_period UNIQUE (tenant_id, fiscal_year, period_no),
    CONSTRAINT ck_tax_filing_period CHECK (period_no BETWEEN 1 AND 12),
...
CREATE TABLE fin_consolidation_runs (
    ...
    combined_revenue numeric(14,2) NOT NULL,
    combined_expense numeric(14,2) NOT NULL,
    intercompany_revenue numeric(14,2) DEFAULT 0 NOT NULL,
    intercompany_expense numeric(14,2) DEFAULT 0 NOT NULL,
    consolidated_profit numeric(14,2) NOT NULL,
    entity_payload varchar(10000) NOT NULL,
    status varchar(24) DEFAULT 'DRAFT' NOT NULL,
    snapshot_id uuid,
    ...
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    created_by varchar(64), updated_by varchar(64), version bigint DEFAULT 0 NOT NULL,
    CONSTRAINT ck_consolidation_period CHECK (period_no BETWEEN 1 AND 12),
    CONSTRAINT ck_consolidation_entities CHECK (entity_count >= 2),
...
CREATE TABLE fin_voucher_generation_requests (
    ...
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    created_by varchar(64), updated_by varchar(64), version bigint DEFAULT 0 NOT NULL,
    CONSTRAINT uk_voucher_request_key UNIQUE (tenant_id, idempotency_key),
    CONSTRAINT ck_voucher_request_status CHECK (status IN ('PENDING','PROCESSING','SUCCEEDED','FAILED','COMPENSATED'))
);
...
CREATE INDEX idx_cash_scenario_asof ON fin_cash_forecast_scenarios (tenant_id, as_of_date DESC);
...
CREATE INDEX idx_report_snapshot_scope ON fin_report_snapshots (tenant_id, report_type, scope_key, captured_at DESC);
...
VALUES
    ...
    ('default','1701','无形资产','ASSET','DEBIT',false,true,true),
    ('default','1702','累计摊销','ASSET','CREDIT',false,true,true),
    ('default','2201','应付账款-暂估','LIABILITY','CREDIT',false,true,true)
ON CONFLICT (tenant_id, code) DO NOTHING;

INSERT INTO sys_permissions (id, tenant_id, code, name, module, created_at, updated_at, built_in, version)
VALUES
    ('00000000-0000-4000-8000-000000005012','default','finance:operations:view','财务运营工作台查看','finance',now(),now(),true,0),
    ('00000000-0000-4000-8000-000000005013','default','finance:operations:manage','财务运营处理','finance',now(),now(),true,0)
ON CONFLICT (tenant_id, code) DO NOTHING;
...
WHERE role.code IN ('ADMIN','FINANCE_MANAGER','FINANCE_ACCOUNTANT')
  AND permission.code IN ('finance:operations:view','finance:operations:manage')
